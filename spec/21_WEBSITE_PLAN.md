...
**Success metrics.** (a) Install-command copy events and `docs/` pageviews as the OSS funnel; (b) waitlist sign-ups as the commercial funnel; (c) time-to-first-meaningful-paint under 1.5 s and a Lighthouse performance score ≥ 95 (a developer tool's site that is slow undercuts the pitch); (d) zero-cookie, zero-consent-banner analytics coverage. No vanity metrics — bounce rate on a single-page install site is meaningless.
...
- **Recommendation: Cloudflare Web Analytics** — free, cookieless, no client-side state, and already in the stack since the site is on Cloudflare Pages. It needs no consent banner under GDPR/ePrivacy because it sets no cookies and does no cross-site tracking. This is the zero-friction default.
...
- **Waitlist data.** Treat emails as PII: store minimally (email + timestamp + source), never share, disclose usage plainly on the form ("we'll email you once about the hosted tier"), and keep the store (Cloudflare KV/D1 or the form provider) access-controlled. Honor deletion requests. The privacy stance is a feature — state it on the site, don't bury it.

## 8. SEO & launch

**On every page:** descriptive `<title>` and meta description, canonical URL, OpenGraph + Twitter card tags, and a generated `sitemap.xml` + `robots.txt` (Astro/Starlight emit these). Structured data (`SoftwareApplication`) on Home so the CLI surfaces well. Docs pages get per-page titles keyed to commands ("devstrap sync", "self-hosting a hub") to win the long-tail queries developers actually type.
