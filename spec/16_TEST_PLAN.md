...
- installer verification gate (`P7-QUAL-02`): the Ubuntu lint job runs `shellcheck scripts/install.sh`, and a push-to-`main`-only `installer-smoke` matrix on Ubuntu/macOS resolves the latest published release, installs cosign, runs the installer with a pinned `DEVSTRAP_VERSION` into `$RUNNER_TEMP/bin`, and executes `devstrap version`; the Ubuntu leg additionally strips cosign from `PATH` with no `DEVSTRAP_INSTALL_CHECKSUM_ONLY` hatch and requires the installer to refuse;
