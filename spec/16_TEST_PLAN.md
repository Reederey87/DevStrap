...
- Windows first-pass smoke (`P4-QUAL-04`): a separate advisory `windows-latest` job (`continue-on-error` for its first cycle) builds and vets `./...`, then tests the platform-safe `internal/platform`, `pathkey`, `ignore`, `draftbundle`, `envfile`, `redact`, `id`, and `pairing` packages. `internal/git` is excluded (its fake-`git`-executable test fixture is Unix-`PATH`-only, discovered on the job's first real run — a test-infrastructure gap, not a production bug). It intentionally excludes the Unix-assumption-heavy CLI/hub/state suites too, so this is build/vet/narrow-test visibility rather than a full-Windows-support claim;
