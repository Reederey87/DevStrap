...
- Windows first-pass smoke (`P4-QUAL-04`): a separate advisory `windows-latest` job (`continue-on-error` for its first cycle) builds and vets `./...`, then tests the platform-safe `internal/platform`, `pathkey`, `ignore`, `git`, `draftbundle`, `envfile`, `redact`, `id`, and `pairing` packages. It intentionally excludes the Unix-assumption-heavy CLI/hub/state suites, so this is build/vet/narrow-test visibility rather than a full-Windows-support claim;
