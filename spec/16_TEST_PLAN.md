...
- Windows first-pass smoke (`P4-QUAL-04`): a separate advisory `windows-latest` job (`continue-on-error` for its first cycle) builds and vets `./...`, then tests the platform-safe `pathkey`, `ignore`, `draftbundle`, `envfile`, `redact`, `id`, and `pairing` packages plus only this finding's own tests within `internal/platform` (`-run 'TestProcessAlive|TestRuntimeGOOSBranchesStayInPlatformPackage'`). `internal/git` is excluded (its fake-`git`-executable test fixture is Unix-`PATH`-only). The rest of `internal/platform` is not run wholesale: most of its golden tests hardcode Unix-style absolute paths and fail on Windows (a pre-existing gap, tracked as a follow-up, unrelated to `ProcessAlive`). It intentionally excludes the Unix-assumption-heavy CLI/hub/state suites too, so this is build/vet/narrow-test visibility rather than a full-Windows-support claim;
...
9. learner-owed rotation (P7-SYNC-04): `internal/state/wck_rotation_test.go` (`SetWCKRotationPendingTx` arms at epoch>0, epoch-0 no-op, storm-guard leaves an existing marker untouched, `CurrentKeyEpochTx` matches `CurrentKeyEpoch`); `internal/sync/wck_rotation_owed_test.go` (remote `device.revoked`/`device.lost` apply arms the marker at the active epoch, snapshot-import flip arms it, keyless epoch-0 device flips trust but never arms, and the storm-guard — replay never touches the marker, a later distinct flip preserves the original Since); the existing `internal/cli/wck_rotation_test.go` gate tests then consume the shared marker (rotate-and-clear, survive-failure, fail-closed on malformed); e2e coverage rides `sync_never_granted_epoch_wedge.txtar` (item under `P6-SEC-03` above)
```
...
starting an independent fsmonitor daemon. Note what the production `Runner` test
does and does not cover: `TestSecureArgsDisablesDetachedAutoMaintenance` pins the
ARGUMENT POLICY (both detach keys present, and `gc.auto`/`maintenance.auto`
absent). It does not demonstrate the runtime behavior — a behavioral test was
written and deleted because it passed with the fix reverted, and the behavioral
claim rests on the recorded `GIT_TRACE` evidence in `spec/18` instead.
...
- `ProcessAlive` is exercised without OS-specific external commands by re-executing the current Go test binary: the helper is alive while blocked on stdin, dead after stdin closes and `Wait` reaps it, and a sentinel nonexistent PID is dead. The production adapter is build-tagged: signal 0 on Darwin/Linux, `OpenProcess` + `GetExitCodeProcess` on Windows, and a conservative alive stub elsewhere;
- agent OS sandbox (`P4-GIT-03`): SBPL profile shape golden tests run on every platform (`TestSBPLProfileConfinesWritesAndQuotesPaths`, quoting/escaping, optional-deny omission, and `TestSBPLProfileEmbedsViolationTag` for per-run Seatbelt denial messages); pure Seatbelt-denial parsing is pinned by `TestParseSeatbeltDenials` and `TestParseSeatbeltDenialsSkipsEmptyAndGarbage`; bubblewrap argv-shape tests run on every platform (`TestBwrapArgsConfinesWritesAndMasksSensitivePaths`, plus optional-piece omission and Seatbelt-deny-list mirror companions); Landlock pure contract tests run on every platform (`TestSandboxHelperArgsShape`, `TestLandlockLimitationsPerABI`, `TestLandlockLimitationsMirrorBwrapOnlyGuarantees`); the CLI mode-x-policy-x-availability matrix runs against a fake adapter (`TestResolveAgentSandboxMatrix` — auto/require/off, yolo-local conflict, readonly/cautious network deny) with capability handling pinned by `TestResolveAgentSandboxCapabilities`; telemetry helper/collection coverage is pinned by `TestSandboxTelemetryHelpers`, `TestCollectSandboxViolationsPersistsScrubbedRows`, store round trips `TestAgentRunSandboxColumnsRoundTrip`/`TestSandboxViolationsRoundTripAndCount`, and doctor grading `TestCheckSandboxViolations`; the sandbox-ENABLED exec branch of `runAgentProcess` (per-run TMPDIR create/repoint/teardown, argv wrapping, adapter cleanup) is pinned by a deterministic cobra-level test with a recording passthrough fake (`TestAgentRunSandboxEnabledExecPath`, all platforms — the kernel enforcement tests bypass this glue); the hidden shim's CLI surface is pinned by `TestSandboxHelperCommandIsHiddenAndFailsClosed`; darwin profile write/cleanup is exercised unconditionally (`TestSeatbeltCommandWrapsArgvAndCleansUpProfile`), and credential-deny leaf-symlink resolution (the profile denies both the literal `~/.ssh` alias and its resolved target, absent anchors keep their literal deny) is pinned by `TestSeatbeltResolvesCredentialLeafSymlinks`; Linux selection and wrapping/cleanup are exercised by `TestChooseLinuxSandbox`, `TestBubblewrapCommandWrapsArgvAndCleanupIsSafe` (with bwrap-less guard tests for empty/dash argv), `TestLandlockCommandRejectsEmptyArgv`, `TestLandlockCommandWrapsArgvWithHelper`, and `TestExecSandboxHelperGuards`; the seccomp syscall denylist is pinned on every platform by the golden-set/deliberately-allowed test `TestSeccompDeniedSyscallNames` and the bwrap `--seccomp` fd argv-shape test `TestBwrapArgsSeccompFD`, and on Linux by `TestSeccompFilterProgramAssembles` (clean per-arch assembly, 8-byte-aligned sock_filter round-trip) and `TestSeccompPolicyFiltersUnknownArchNames` (x86-only names dropped on arm64 so assembly never fails); read confinement is pinned on every platform by `TestReadConfineRoots` (per-OS allow-list roots, credential dirs excluded, absolute-extra dedup), the SBPL ordering test `TestSBPLProfileReadConfineOrdering` (deny-all → metadata-allow → root-allow → credential-deny last, so a credential inside an allowed root stays denied) with `TestSBPLProfileReadConfineOffIsUnchanged` keeping the non-confined profile byte-identical, the bubblewrap shape test `TestBwrapArgsReadConfineEnumeratesRootsAndSkipsMasks` (`--ro-bind-try` roots replace `--ro-bind / /`, masks skipped), and the CLI resolution matrix `TestResolveAgentSandboxReadConfineMatrix` (auto/on/off × policy × can-enforce, explicit-on and require refuse when unenforceable, typo fails closed), the fail-closed guards `TestResolveAgentSandboxReadConfineExplicitOnRefusesWhenUnavailable` (explicit on with no sandbox refuses) and `TestResolveAgentSandboxReadAllowCredentialConflictRefuses`/`TestFirstReadAllowCredentialConflict` (a `--read-allow` root overlapping a credential path is refused so it cannot re-expose it on bwrap/Landlock); the Landlock e2e additionally proves read confinement kernel-denies a credential read that is otherwise allowed while keeping worktree reads working; and real kernel enforcement (outside-write blocked, raw `truncate(2)` blocked under the Landlock v3 floor, `/dev/null` allowed, log dir blocked, credential read denied/masked only by full-fidelity backends — the Seatbelt e2e reads the key both through a `~/.ssh` symlink and via its resolved target and asserts both kernel-denied — exit-code fidelity through the Landlock shim, a denied `keyctl(2)` returning EPERM plus an over-blocking `git init`+commit canary under both Linux backends' seccomp filters, and network denied when requested and supported) is env-gated behind `DEVSTRAP_SANDBOX_E2E=1` (`TestSeatbeltSandboxEnforcement`, `TestBubblewrapSandboxEnforcement`, `TestLandlockSandboxEnforcement`; CI installs bwrap on ubuntu-latest, relaxes the AppArmor userns sysctl, hard-fails the bwrap runner probe, and adds a Landlock hard-fail runner probe plus real-binary `sandbox-helper` smoke so in-test skips cannot mask regressions);
- polling watcher emits advisory scan events and stops on context cancellation;
- the run-loop service manager (`P4-PROD-04`) is pinned on every platform by the untagged pure-logic tests: `TestRenderLaunchdPlistGolden`/`…EscapesXML`/`…OmitsOptionalKeys`, `TestLaunchdArgvBuilders`, `TestParseLaunchctlPrint`, `TestRenderSystemdUnitGolden`, `TestSystemdQuoteEscapesSpacesAndPercent`, and `TestSystemdArgvBuilders`; the CLI surface runs against a fake `ServiceManager` injected through the `serviceBackend` seam (`TestServiceInstallBuildsRunLoopArgs`, `…RefusesTempExecPath`, `…RequiresHubConfig`, `…PrintsAdapterNotes`, `…EnvContainsNoSecrets`, `TestServiceUninstallIdempotent`, `TestServiceStatusJSON`, `TestServiceUnsupportedExitsNonzero`, and `TestDoctorWarnsWhenServiceInstalledButStopped`);
- the fallback-platform (`!darwin && !linux`) service/keychain placeholders still return the `ErrUnsupported` sentinel (`TestUnsupportedAdaptersReturnSentinel`); the launchd and systemd-user managers are now shipped adapters, not placeholders;
...
`db backup --full` (state.db + blobs + key material) and the `db restore` path are SHIPPED (`P6-DATA-04`, hardened by `P7-DATA-03/04/05`: snapshot-authority enumeration, hash manifest + fail-closed verification, journaled all-or-nothing promotion with resumable recovery). The recovery drill:
...
- manifest round trip: export workspace.yaml, wipe, import, and assert the namespace map is equivalent   [SHIPPED W13-02]
```

**The plain-text workspace manifest SHIPPED 2026-08-01 (`W13-02`)**, closing this DIRECTION's export/import half and its "document recovering the namespace without DevStrap" half (`../docs/recovery.md`). `devstrap export --manifest <path> [--pinned]` and `devstrap import --manifest <path>` are specified in `13_CLI_DAEMON_API.md` § *Workspace manifest commands (AD-7)*; the emitted document is a `vcstool` `.repos` file, so an unrelated binary can rebuild the tree.

**The interop claim is scoped, and the scoping is part of the artifact.** It holds for `git_repo` projects **only**: `local_git`, `plain_folder` and `draft_project` rows have no `{url, version}` and are structurally invisible to `vcs import`, and draft/env content is age-encrypted to device recipients and recoverable only through DevStrap by design. That subset is the bulk of the DR value, but the emitted manifest's own header says so — a user reading the file mid-disaster must not have to find this spec — and so does this row. **The other two drills are NOT in scope for `W13-02` and remain future work**: total-hub-loss is unbuilt, and total-local-loss is automation of shipped `db restore` machinery (`db_restore_journal_recovery.txtar` already drives the interrupted-restore path). An earlier draft folded total-local-loss into the manifest item, which would have quietly re-scoped a `db restore` drill as manifest work.

Coverage for the shipped half, asserted at both levels the drill needs:

- `cmd/devstrap/testdata/script/manifest_roundtrip.txtar` runs the drill through the **real binary**: build a git project plus a non-git draft project, export, delete both the state dir and the managed root, `init`, `import`, assert `status` lists both projects again, then `materialize` and assert the checkout is back. It also pins the machine contract — one JSON document on stdout, `exitstatus 1` for a partial import, `exitstatus 2` for a file that is not a manifest — and greps the emitted file for the scoping and no-secrets lines, so deleting them from the artifact fails the build.
- `internal/manifest`: `TestEncodeMatchesGolden` pins the exact emitted bytes against `testdata/workspace.golden.yaml`, and `TestGoldenStaysVCSToolCompatible` reads that golden as a **generic YAML tree** — never through this package's own structs, which would validate the implementation against itself — asserting the root `repositories` mapping, `type`/`url` on every entry, and that no DevStrap key ever appears inside an entry vcstool parses. `TestVCSToolImportsEmittedManifest` runs the real `vcs import` against local bare repos (network-free, skipped when `vcstool` is absent) and additionally asserts a `plain_folder` is **not** reconstructed, so the honest half of the scoping is executable too.
- `internal/cli`: `TestManifestRoundTripRecoversNamespaceAfterTotalLoss` (row-level equivalence), `TestImportRegistersWithoutMaterializing` + `TestImportedProjectMaterializesThroughTheExistingPass` (the plane boundary from both sides), credential stripping, `--pinned` omission rather than degradation, the conflict refusal, the foreign-VCS skip, unsafe paths, a newer `schema_version`, and both commands' stdout/stderr separation. `TestExportPinnedScopesReachabilityToTheExportedRemote` (`P11-MANIFEST-01`) builds the fork scenario — HEAD pushed to a second remote and NOT to the registered one — and asserts the entry carries no `version`, then that the pin returns once the registered remote genuinely serves the commit, so the gate is proven scoped rather than merely stricter. `TestExportPinnedRefusesWhenARemoteIsNestedInsideTheExportedOne` closes the hole under the scoping — a remote *named* `origin/vendor` writes refs the `origin/*` pattern matches — and asserts the precondition (`origin/*` really does list `origin/vendor/main`) first, so the refusal is not proven against a vacuous setup. It builds that remote with `git config`, not `git remote add`, because the porcelain guard against nested names is git-version-dependent (2.50.1 allows it, the CI runners refuse it) while the config write works everywhere; a `remote add` construction passes locally and fails CI. `internal/git`: `TestRemotesReadsEveryConfiguredRemote` (the remote-less repo's empty map, which `git config --get-regexp`'s exit 1 would otherwise surface as an error; a dotted remote name; and first-wins for a multi-url remote, since git fetches from the first url and last-wins would be a FALSE pin) and `TestRemoteTrackingContainsRefusesAnUnscopedQuery` (an empty remote list and an option-injection/glob-metacharacter name are both refused **without spawning git**, proven by a fake-git attempt counter).
...
- **No fuzz targets** for any untrusted-input parser, incl. the env parser, pathkey, and the secret scrubber (`TEST-01`); add `go test -fuzz`. **[Implemented: the env parser (`internal/envfile` `FuzzParseBytes`), the ignore compiler (`internal/ignore` `FuzzCompile`), the convergence decider (`internal/sync` `FuzzDecideConvergence` — present earlier; this row never named it), `internal/pathkey` (`FuzzClean`, `W13-07`) and the secret scrubber (`internal/redact` `FuzzRedactorNeverLeaks` + `FuzzWriterSplitBoundaries`, `W13-06`). **Both new targets found real behaviour on their first run.** `FuzzClean` found a production defect in `path_key` derivation and then drove a decision NOT to adopt full case folding — see `07_NAMESPACE_AND_SYNC_MODEL.md` § *Path identity*. The scrubber targets found a limitation recorded and deliberately NOT patched: `redact.Writer` scrubs one complete line at a time, so a registered secret spanning a line boundary is forwarded verbatim (PEM blocks are already covered by `SECU-04`'s suppression, so the residual is a non-PEM multi-line value). Both targets also skip below `AddValue`'s documented 4-byte floor — asserting against that guard would assert the opposite of the intended contract.]**
- **e2e harness covers only `init`/`status`** (`TEST-02`); the riskiest flows (scan/hydrate/worktree/agent/env/sync) are tested in-process and bypass the real exit-code/`--json` contract. Extend the `rogpeppe/go-internal` testscript suite. **[Largely implemented: testscripts now cover clone/doctor/draft/materialize/run-loop/encrypted sync/wip and — since 2026-07-25 — the daemon lifecycle; remaining gaps: worktree/agent/env flows through the real binary.]**

**Daemon lifecycle e2e (2026-07-25).** `cmd/devstrap/testdata/script/daemon_lifecycle.txtar` is the suite's **first script to run a background process**, and it exists to cover what the in-process daemon tests structurally cannot: the real process exit code. It asserts not-running status → background `daemon start` → the socket appears → status reports running *over the socket* → `daemon stop` → the socket is unlinked → `daemon events` exits **3**. Three harness notes, each load-bearing:

- **`exitstatus N <cmd...>`** (new `Cmds` entry) asserts an *exact* exit code. Named `exitstatus`, not `status`, so it can never be shadowed by a future go-internal builtin (builtins win the lookup) and never reads as `devstrap status`. testscript's `!` only proves non-zero, and `exitDaemonUnavailable = 3` — reserved since before the daemon wave and first returned by `daemon events` — is precisely the thing worth pinning; a test that accepted any failure would also pass on a panic.
- **`waitfile`** (new `Cmds` entry) polls for the socket instead of sleeping. The daemon binds asynchronously after the process starts, so a fixed sleep is either flaky or slow.
- **`Setup` exports `$SHORTHOME`**, a short `os.MkdirTemp` dir, because the daemon's socket is `<home>/devstrapd.sock` and testscript's `$WORK` (`<GOTMPDIR>/go-test-scriptNNNNNNNNN/script-<name>/`) pushes that past `sockaddr_un.sun_path`'s 104-byte darwin cap. The resulting `EINVAL` is deliberately *not* mapped to `ErrUnavailable` by `internal/daemon/client.go`, so the failure would surface as a confusing transport error rather than "no daemon". Same precedent as `tempSocketPath` in `internal/daemon/daemon_test.go` and the short-temp-dir note in `internal/cli/daemon_test.go`.

The script also incidentally pins two behaviors the daemon's docs assert: `daemon start` exits **0** on `SIGTERM` (its `wait daemon` line carries no `!`), and the socket is reliably gone once that process has exited — stable across repeated `-race` runs.
- **Coverage profile is computed then discarded** and the vacuous-test guard checks only 3 packages (`TEST-03`). **[The coverage-profile half shipped earlier (`P5-QUAL-04` / `P7-QUAL-05`): CI consumes `coverage.out` and enforces a 50% total floor, plus per-package floors via `go-test-coverage` against `.testcoverage.yml` — this row was never updated when that landed. The vacuous-test-guard half closed 2026-08-01 (W13-06): the guard now enumerates every `go list ./internal/... ./cmd/...` package and fails if any lacks a `*_test.go` file. Deliberately deferred: the stronger "non-zero passing-test count" form (needs `go test -json` plumbing). `internal/id` tests from the P4-SEC-07 pairing work remain.]**
...
- **`govulncheck` is unpinned (`@latest`) and bundled into the "Go tests" job** (`TEST-05`/`CI-01`); pin it and split it into its own (non-blocking/scheduled) job. **[Implemented 2026-06-28: pinned `@v1.1.4`, split into own `vuln` CI job, `continue-on-error` on PRs, daily scheduled run.]** **[2026-07-10: bumped the Go toolchain pin in `go.mod` to `go 1.26.5` (all CI/release jobs resolve it via `go-version-file: go.mod`) to clear `GO-2026-5856`, a `crypto/tls` standard-library vulnerability fixed in go1.26.5 that was failing the `vuln` gate on every branch including `main`.]** **[2026-07-10: bumped `golang.org/x/crypto` v0.51.0→v0.52.0 and `golang.org/x/net` v0.54.0→v0.55.0 (Dependabot #143/#126 rewritten as one manual PR — Dependabot cannot satisfy this spec-drift gate); `go test -race ./...` and `govulncheck` clean.]** **[2026-07-24: bumped `golang.org/x/text` v0.38.0→v0.40.0 (`golang.org/x/tools` v0.45.0→v0.47.0 transitively) to clear `GO-2026-5970` (infinite loop on invalid input in `x/text`), which was failing the `vuln` gate on every open branch and blocking the working-state wave's merge cascade; `go test -race ./...` and `govulncheck` clean.]**
- **The fsnotify watcher has no tests and concurrent code has no goroutine-leak detection** (`TEST-06`). **[Implemented 2026-07-24: `internal/platform/fsnotify_watcher_test.go` is the adapter's first coverage, added to satisfy the Milestone 5 entry gate (`14_MVP_ROADMAP_AND_BACKLOG.md`). `TestNativeWatcherCoalescesBurstIntoBoundedHints` is the indexer-hydration-storm case — 3000 writes must collapse to a handful of hints, and every `FSEvent` must carry `Kind=FSEventScan`/`Path=<watch root>`, the structural form of "events are hints, not hydrate instructions". `TestNativeWatcherRestartAfterBulkChangeMakesNoCompletenessClaim` is the *Sleep/wake simulation* case below, and deliberately asserts only that a restarted watcher works — never that offline changes are reported — pinning that periodic reconciliation, not the watcher, carries the completeness guarantee (that half is proven separately by `cmd/devstrap/testdata/script/run_loop_once.txtar`, `P6-XP-03`). `TestNativeWatcherStopReleasesGoroutinesAndReturnsContextError` closes the goroutine-leak half by polling `runtime.NumGoroutine()` back to baseline after cancel (hand-rolled rather than `goleak`, so no new dependency). `TestNativeWatcherBlocksOnSlowConsumerAndUnblocksOnCancel` pins the backpressure contract a daemon consumer must honor — the flush path blocks on an unread channel and is released only by context cancel — and `TestShouldSkipWatchDir` tables the prune set. **[Updated 2026-07-28 (`M5D-06`): `TestShouldSkipWatchDir` is gone — it exercised a helper with no production caller, so its prune assertions guarded nothing. `TestWatchSetIsExactlyTheCompilerSurvivors` replaces it and asserts the WATCH SET rather than hint counts, which is the only form that catches the `PLAT-01` defect: the file-event filter suppresses hints from inside an ignored directory even while that directory is still watched, so a hint-based test stays green when the exclusion set regresses to the old hardcoded four entries (mutation-checked — it registers 13 watches instead of 3). Three more tests join it: `TestWatcherSurvivesTransientDirectories` (a directory created and deleted before the walk reached it returned ENOENT, which was terminal — so any `go test`/`npm` run inside a watched project killed the watcher within seconds and it re-died on every retry); `TestWatcherDropsWatchesForDirectoryRenamedOutOfTree` (descriptor release; it renames rather than deletes because both kernels drop a watch when the inode is unlinked, so a delete-based test passes with or without the fix); and `TestWatcherReportsDeletionOfFileNamedLikeAPrunedDirectory` plus `TestEventPathUnderRoot` — the latter pinning a guard against the backend's own output, since fsnotify's kqueue backend emits events with an empty or `.` Name after a watch is removed, and `.` resolves against the process working directory.]** `PollWatcher` is not re-covered here: `TestPollWatcherEmitsScanAndStopsOnContext` in `platform_test.go` already pins its hint shape and cancellation. Writing these surfaced and fixed a real defect **in this same change**: the create-event branch passed each newly-created directory to `addRecursiveWatch` as that call's own walk root, where the prune set does not apply, so a fresh `node_modules`/`.git` registered a watch per subdirectory and one failing (terminal) `watcher.Add` could kill the watcher outright — mutation-checked, the test fails when the fix is reverted.]**
