...
- agent OS sandbox (`P4-GIT-03`): SBPL profile shape golden tests run on every platform (`TestSBPLProfileConfinesWritesAndQuotesPaths`, quoting/escaping, optional-deny omission); bubblewrap argv-shape tests run on every platform (`TestBwrapArgsConfinesWritesAndMasksSensitivePaths`, plus optional-piece omission and Seatbelt-deny-list mirror companions); the CLI mode-x-policy-x-availability matrix runs against a fake adapter (`TestResolveAgentSandboxMatrix` — auto/require/off, yolo-local conflict, readonly/cautious network deny); the sandbox-ENABLED exec branch of `runAgentProcess` (per-run TMPDIR create/repoint/teardown, argv wrapping, adapter cleanup) is pinned by a deterministic cobra-level test with a recording passthrough fake (`TestAgentRunSandboxEnabledExecPath`, all platforms — the kernel enforcement tests bypass this glue); darwin profile write/cleanup is exercised unconditionally (`TestSeatbeltCommandWrapsArgvAndCleansUpProfile`); Linux wrapping/cleanup is exercised when bubblewrap is available (`TestBubblewrapCommandWrapsArgvAndCleanupIsSafe`, with bwrap-less guard tests for empty/dash argv); and real kernel enforcement (outside-write blocked, `~/.ssh` read blocked/masked, confined work allowed, network denied when requested) is env-gated behind `DEVSTRAP_SANDBOX_E2E=1` (`TestSeatbeltSandboxEnforcement`, `TestBubblewrapSandboxEnforcement`; CI installs bwrap on ubuntu-latest, relaxes the AppArmor userns sysctl, and hard-fails a runner probe so the in-test skip cannot mask regressions);
