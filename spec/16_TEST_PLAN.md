...
- **No fuzz targets** for any untrusted-input parser, incl. the env parser, pathkey, and the secret scrubber (`TEST-01`); add `go test -fuzz`. **[Partially implemented: fuzz targets shipped for the env parser (`internal/envfile`), the ignore compiler (`internal/ignore`), the convergence decider (`internal/sync`, `FuzzDecideConvergence` — the row never listed it), and, since `W13-07` (2026-08-01), `internal/pathkey` (`FuzzClean`). **`FuzzClean` found a real defect on its first run**: `path_key` was derived with `strings.ToLower` after NFC, so `"Y"+U+030A` and `U+1E99` — and `U+0130` and `"i"+U+0307` — produced DIFFERENT keys for what a case-insensitive filesystem calls one directory, and `DetectCaseConflicts` therefore did not fire. Fixed by folding (`cases.Fold`) and re-normalizing after. The secret scrubber (`internal/redact`) remains.]**
