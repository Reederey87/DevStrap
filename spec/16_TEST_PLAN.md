...
- the `scan --adopt` workspace-root gate (`P6-CLI-02`): out-of-root `--adopt` refuses with `exitUsage` and adopts nothing, an explicit positional root equal to the workspace root still adopts, and read-only scans of foreign directories keep working;
