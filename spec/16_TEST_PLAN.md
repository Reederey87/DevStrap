...
- **No fuzz targets** for any untrusted-input parser, incl. the env parser, pathkey, and the secret scrubber (`TEST-01`); add `go test -fuzz`. **[Partially implemented: shipped for the env parser (`internal/envfile` `FuzzParseBytes`), the ignore compiler (`internal/ignore` `FuzzCompile`), the convergence decider (`internal/sync` `FuzzDecideConvergence` — present earlier; this row never named it), and `internal/pathkey` (`FuzzClean`, `W13-07`). **`FuzzClean` found a real defect on its first run** — `path_key` normalized only BEFORE case mapping — and then drove a decision NOT to adopt full case folding, because enumeration showed folding merges pairs the filesystem keeps distinct; see `07_NAMESPACE_AND_SYNC_MODEL.md` § *Path identity*. The secret scrubber remains.]**
