...
**The benefit is real but unquantified.** fsnotify's macOS backend is kqueue, which needs an open file descriptor per watched path and — because kqueue has no directory-content notification — additionally one per entry inside each watched directory, so descriptor usage tracks the *file* count under `~/Code`, not the directory count. `addRecursiveWatch` registers the whole managed root at start and on every directory create; since `M5D-06` it prunes through the compiled ignore matcher — the watch root's `.devstrapignore` plus canonical defaults — rather than the original hardcoded four entries, and it now *releases* watches for subtrees that leave the namespace, so the count tracks real source content instead of growing for the life of the process (`PLAT-01`/`PLAT-04`, shipped). That lowers the kqueue bill without changing its shape: it is still one descriptor per watched path. FSEvents by contrast is one per-session stream subscribed to a path prefix, with no per-path descriptor, and it survives sleep/wake and volume remount through a replayable event id. Whether the kqueue cost actually bites depends on the size and shape of a real `~/Code` and on a per-process descriptor limit that varies by macOS version and launch context — not a constant this spec can assert.

**The consumer now exists; the measurement is still owed.** This paragraph previously read "we have no measurement, because the watcher has no consumer" — true when it was written, and stale since the daemon's watch plane became `NativeWatcher`'s first production consumer. `/v1/health`'s `watch.roots` is the instrument. What is still missing is a reading from a real `~/Code`, which `M5D-06` is a prerequisite for: pruning through the compiler and releasing departed subtrees both move the number, so a measurement taken before this change would have described a watcher that no longer exists. Choosing FSEvents today would still be guessing at a cost we have not observed.
...
**Reconsider when** either holds: (a) measured descriptor usage on a representative `~/Code` approaches the process limit even now that the `spec/11` ignore compiler feeds watcher exclusions (`PLAT-01`, shipped `M5D-06`) — i.e. the pruning fix is exhausted and the backend is still the constraint; or (b) the kqueue backend demonstrably drops or duplicates events across sleep/wake or volume remount in a way periodic reconciliation cannot absorb. Either finding makes cgo-on-darwin (or a separately shipped native helper) worth its distribution cost. Neither is demonstrated today.
