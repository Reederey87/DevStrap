...
Env profile replay is LWW by the profile row's source-event coordinate: the highest `(HLC, device_id, event_id)` wins, matching namespace reconciliation. Apply soft-skips an env event whose project is absent (for example after a winning tombstone); a later re-capture re-emits the pointer.
