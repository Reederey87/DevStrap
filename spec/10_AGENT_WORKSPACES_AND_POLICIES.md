...
The cloud-sync architecture (eager blobless clone of repo content, age-encrypted env/draft blobs, the signed HLC-ordered namespace map, and the cross-machine working-state plane — git-state validation + `refs/devstrap/wip/*`) is purely a *human* device-mirroring plane. It must **never** feed agent base resolution. Regardless of what the sync plane has materialized or observed on this device — including a WIP ref already sitting in the SAME local repo a fresh worktree is about to be cut from (the adversarial precondition, not merely a ref nobody fetched), or a hostile/misconfigured remote's HEAD symref repointed at a non-branch ref while the real default branch still exists right next to it — this invariant is enforced and regression-tested: `RemoteDefaultBranch`/`symbolicOriginHead` (`internal/git/git.go`) require their resolved HEAD symref target to sit under `refs/heads/`/`refs/remotes/origin/` respectively before trusting it as a branch name, rather than blindly stripping a well-known-looking prefix from whatever the remote or a local ref reports; `internal/git`'s `TestRemoteDefaultBranchRejectsNonBranchSymref`/`TestSymbolicOriginHeadRejectsNonRemoteTrackingTarget`/`TestResolveDefaultBranchAdversarialHeadSymrefWithRealMainAlsoPresent`/`TestDefaultBranchResolutionErrorsWhenRemoteOnlyHasWipRef`/`TestDefaultBranchResolutionIgnoresLocalWipRefAlongsideDefaultBranch` (the last inspects the actual git subprocess argv the resolution+fetch flow issues) and the e2e `worktree_never_bases_off_wip_ref.txtar` (two devices, a real `wip push`/`wip fetch` round trip, then a fresh `worktree new --fresh-upstream` proven to omit the WIP content and record a base sha distinct from the WIP push, with the WIP ref itself left untouched) pin it:
...
### Adopting externally-created worktrees (`AD5-02`)

`devstrap worktree new --fresh-upstream` is only half the story: modern agent harnesses (Claude Code, Cursor, Codex, Devin) routinely create their own linked git worktrees, entirely outside DevStrap. `devstrap worktree adopt <path>` (see `spec/13_CLI_DAEMON_API.md` § *Worktree commands* for the full CLI contract) closes that gap by registering such a worktree after the fact — but it is strictly a **registration** plane, never a **base-resolution** plane, and the invariant above governs it exactly as it governs `worktree new`: adoption records what a worktree was *actually* based on and never rewrites, repairs, or blesses that base, and nothing it stores is ever read by the fresh-worktree resolver.

The mechanism lives in `internal/git/git.go` alongside `RemoteDefaultBranch`/`symbolicOriginHead`:

- `Runner.WorktreeIdentity` is the single definition of "is this a linked worktree, a main checkout, or nothing at all" — extracted from what was previously private logic inside `WorktreeSandboxWriteDirs` (that sandbox-grant resolver now calls it too, so there is exactly one place that decides worktree identity). It resolves `CommonDir`/`GitDir` (symlink-resolved, matching the sandbox resolver's existing symlink handling), classifies `IsLinked`, derives `MainCheckout`, and reads `Branch`/`HeadSHA` — `Branch == ""` for a detached HEAD (the common, EXPECTED shape for an agent-harness worktree, adopted rather than refused) and `HeadSHA == ""` for an unborn HEAD (refused — there is nothing to merge-base against).
- `Runner.MergeBase` resolves the actual common ancestor of the worktree's `HEAD` and the target base ref, so the recorded `base_sha` is a real merge-base — **never** the base ref's raw tip, which would make every adopted worktree report "fresh" forever and would also corrupt `agent pr`'s `Committed since base:` diff. Two histories sharing no common ancestor (an orphan branch such as `gh-pages` is the ordinary case) is git's own exit-1/empty-output convention, classified as the distinct sentinel `ErrNoMergeBase` rather than a bare failure, so the CLI can refuse with a `--base-ref` remedy instead of a confusing error.
- `Runner.IsShallow` gates adoption on the source repository not being a shallow clone (unless `--allow-shallow`), because a grafted history can make `MergeBase` return a plausible-but-wrong answer right at the shallow boundary — silently corrupting the recorded provenance.

`idx_worktrees_active_path` (`spec/12_DATA_MODEL_SQLITE.md`, migration `00032`) enforces the "one row per physical worktree" invariant at the database level, scoped to `status = 'active'` so a removed worktree's path remains legitimately reusable; both `worktree new` and `worktree adopt` normalize the stored path via `filepath.EvalSymlinks` so a `/var` vs `/private/var`-style alias cannot defeat that uniqueness. Re-running `adopt` on a row it previously created refreshes `base_ref`/`base_sha`/`dirty_state` in place; running it against a row created any other way (e.g. `worktree new`) leaves that row completely untouched — adoption never rewrites a base DevStrap itself already resolved. Reap paths (`worktree cleanup --merged`, `worktree remove`) treat an adopted row as something DevStrap did not create and therefore does not own by default: cleanup skips it unless `--include-adopted`, and remove deregisters the DB row only, leaving the checkout on disk, unless `--prune`.
...
engine: generic  # the ONLY wrapper engine; per-harness adapters are withdrawn, not pending (AD5-05)
task: fix failing tests
...
Wrapper-level enforcement is the guardrail layer *beneath* the shipped OS sandbox (`P4-GIT-03`, 2026-07-05), not perfect sandboxing on its own:
...
- when enabled and supported, an OS-enforced sandbox (macOS Seatbelt, Linux bubblewrap → Landlock+seccomp fallback) confines non-`yolo-local` runs beneath these guardrails; `--sandbox require` fails closed when it cannot be enforced, `--sandbox auto` (default) degrades to advisory-only with a warning if unsupported, and `--sandbox off` disables it; containerization stays the later slice.
...
- **OS-enforced sandbox: macOS Seatbelt, Linux bubblewrap, and Linux Landlock fallback shipped** (`AGEN-03`/`P4-GIT-03`, 2026-07-05). The wraps above are kernel-enforced for the child and everything it spawns — the interpreter bypasses in the first bullet can no longer write outside the worktree; full-fidelity backends also read-deny/mask the denied credential paths and (under `readonly`/`cautious`) deny ordinary network sockets. **Because an agent worktree is a git _linked_ worktree, all three backends additionally grant write to the linked worktree's git storage — the shared object store, refs, and reflogs in the clone's git-common-dir, plus the per-worktree admin dir — so the agent's `git add`/`git commit` no longer fail with a kernel `EPERM` (`P7-SANDBOX-01`, resolved by `git.Runner.WorktreeSandboxWriteDirs`). The common dir's `hooks/` and `config` are deliberately NOT granted: granting them would let the child plant a hook or config that executes UNSANDBOXED on a later git operation. **`P8-SEC-02` (2026-07-31) closed the hole that grant left open**: the per-worktree admin dir was granted *wholesale*, and it contains `commondir` — the pointer to the shared `.git`. A sandboxed agent rewrote it to a directory inside its own worktree, planted a `config` there, and git honoured the relocated common dir, so the next **unsandboxed** `git status` executed an injected `core.fsmonitor` — and DevStrap issues that `git status` itself from `worktree cleanup`'s `DirtyState`, so the escape needed no user action. `SandboxSpec.GitDenyFiles` now carves `commondir`, `gitdir`, and `config.worktree` back out of the grant (safe, because git writes them only at `git worktree add` time): Seatbelt emits a `deny file-write*` **after** the allow block, since SBPL is last-match-wins and a deny emitted first would do nothing; bubblewrap `--ro-bind-try`s them after the rw bind. **Landlock cannot express this** — it is additive-allow and cannot subtract a child from a granted directory, and enumerating siblings instead would break git's own `index.lock`/`MERGE_MSG`/`rebase-merge/` — so the gap is recorded in `Limitations()` and surfaced by `agent run`'s reduced-guarantees notice rather than papered over. These git dirs also join the `--read-confine` allow-list so git reads work under the `readonly` policy; a live Seatbelt e2e (`TestSeatbeltAllowsLinkedWorktreeCommit`) proves the commit fails without the grant and succeeds with it.** **Both Linux backends install a seccomp syscall denylist** (mount/kexec/ptrace/keyring/io_uring/legacy-escape syscalls → `EPERM`; `DEVSTRAP_SANDBOX_SECCOMP=off` opt-out). **`sandbox.violation` telemetry is live as unsigned local visibility**: backend/mode/limitations are recorded per run, macOS Seatbelt denials are collected from tagged unified-log rows after the run, scrubbed, and shown by `agent show`/`doctor`; Linux runtime denial detection remains future. **Tighter read confinement is shipped** (`--read-confine auto|on|off`, env `DEVSTRAP_SANDBOX_READ_CONFINE`, default-on for the `readonly` policy; `--read-allow <abs>` adds roots): all three backends restrict the child's reads to the worktree/tmp, the OS toolchain/system roots, and the `$HOME` build caches instead of the whole disk — the Seatbelt profile denies all reads then re-allows the roots (credential denies stay last so they out-rank the allows; a global `file-read-metadata` allow keeps stat/traversal working), bubblewrap exposes only the roots via `--ro-bind-try`, and Landlock restricts its `RODirs` grant to the roots. Read confinement is a strictly-narrower boundary than the default; even without it the Landlock fallback now denies the credential deny-list by default via leaf-hierarchy grants (`P7-SEC-03`, see below), so credential reads no longer depend on `--read-confine` being engaged. `--sandbox require` refuses to launch if a requested read confinement cannot be enforced. The remaining OS-sandbox direction is containerization (spec/14). Linux caveats: bubblewrap credential masks hide paths (ENOENT/empty) rather than returning EPERM, nested-sandbox tools (Chrome headless, flatpak, bwrap-in-bwrap) can break under `--disable-userns`, and Landlock is a reduced fallback (credential reads ARE denied by default via leaf-hierarchy grants that omit the credential anchors — `EACCES` rather than bubblewrap's empty-mask — but the deny is a set of RODirs/ROFiles leaves rather than a subtraction, so a credential aliased by a non-anchor path or symlink outside the anchor set is not covered; network deny TCP-only at ABI >= 4; no mount/pid namespace; and — because Landlock has no `--new-session` analogue — the `TIOCSTI` terminal-injection escape stays open on the Landlock path, since seccomp does not arg-filter `ioctl`). A kernel without seccomp-filter support degrades to a reduced-guarantees notice, not a hard failure. `--sandbox require` accepts Landlock as a real write-confinement boundary except when the policy demands a network deny that the backend cannot enforce at all (ABI < 4), in which case it fails with the policy exit class before worktree creation; a TCP-only deny (ABI >= 4) passes `require` with a warning that UDP, QUIC, and unix-domain sockets stay open; `--sandbox off` is the explicit escape hatch.
- **The agent deny-list and sandbox credential masks now share canonical credential lists** (`AGEN-05`, shipped 2026-07-30). `internal/ignore` owns the home-directory/home-file membership tables plus the scan/draft secret-name predicate. Checked-in `.env.example`, `.env.template`, and `.env.schema` names are passable to the wrapper (for example, `devstrap agent run -- head .env.example`); a real `.env` token remains denied. `*.key` deliberately stays agent-only because it is a glob over user filenames: promoting it would warn on ordinary names such as `en-US.key` and make draft bundles hard-refuse. The OS sandbox now masks `~/.snowflake` under guarded operation, so a guarded agent driving `snow` cannot use the user's credentials; the explicit escape hatches are `--policy yolo-local` and `--sandbox off`.
...
Current implementation strips credential-bearing inherited env (`SSH_AUTH_SOCK`) and does not expose the user's `HOME`; `HOME` is repointed to the worktree so common dotfile secret locations are not reachable through `~`. This is necessary but not sufficient: if the OS sandbox is unavailable, disabled, or bypassed via `yolo-local`, an agent can still read any path its process user can access if it discovers or constructs that path — the allowlist below governs only what DevStrap itself injects into the child's initial environment, not what the sandboxed (or unsandboxed) process can subsequently read from disk. Treat "no secrets by default" as the wrapper contract, with the Seatbelt/bubblewrap sandbox as the kernel-enforced layer where available.

**Project-env allowlists are shipped** (`P4-GIT-06`): a project opts an `agent run` into exposing its captured/hydrated env profile by committing a `.devstrapagent.yml` at its repository root (parsed by `internal/agentsecrets`, mirroring the `.devstrapignore` project-root-dotfile convention over a DB column — this is a local wrapper-security policy with no cross-device sync requirement). `agent run` reads the file from the **fresh worktree** (fetched from `origin/<default_branch>`, never from local/synced state, per the AD-8 base-resolution rule), so the policy enforced always matches the commit the agent is actually running against. Absence of the file preserves the pre-existing default exactly: no captured profile secrets reach the agent subprocess. Presence of the file — even with an empty or absent `agent_secrets` key — is itself an explicit opt-in; an empty `allow` list still denies everything rather than falling back to "no policy." Only `devstrap_encrypted`-provider profiles are filtered into the child env today; a `1password`-provider profile emits an advisory warning and injects nothing (its refs resolve at runtime through `op run`, which is not yet wired to filter individual keys).
...
Deny wins on conflict: a key listed in both `allow` and `deny` is denied. `Policy.Filter` (`internal/agentsecrets`) builds the allowlist first, then the reserved `DEVSTRAP_AGENT_RUN_ID`/`DEVSTRAP_WORKTREE_ID`/`HOME` overrides are applied last in `runAgentProcess`, so a captured secret can never shadow those.
...
## Agent engines — the adapter path is withdrawn (`AD5-05`, 2026-07-31)

This section previously listed `cursor-cli`, `codex-cli`, and `copilot-cli` as "initial adapters" alongside a planned `AgentRunner` interface. **They are not planned work and never will be built as described.** Leaving them here as "initial" was a promise the project had already decided not to keep — the `AD-5` direction above argued against per-harness adapters, and the substrate primitives shipped instead (`AD5-01`…`AD5-04`).

**The shipped engine set is `generic`, and it is the only *wrapper* engine there will be.** Every other harness is supported by not wrapping it:

- a harness that wants a fresh, provably-based worktree calls `devstrap worktree new --fresh-upstream --json` — a shell command and a JSON parse, which every harness can already do with no integration code;
- a harness that made its own worktree gets registered by `devstrap worktree adopt` and `devstrap agent adopt`, so the registry, the recorded base, and the stale-base gate keep their value regardless of who ran the agent;
- `agent_runs.engine` is therefore a **free-text label the caller supplies** (`claude-code`, `cursor`, whatever it actually is), not a value DevStrap validates against a list. Validating it would reintroduce the per-harness coupling this decision removes.

The user-facing form of all of this is `../docs/agents.md`. The reason an adapter cannot work even in principle is in the `AD-5` direction section: the wrapper strips API keys and repoints `$HOME`, so it cannot authenticate a real harness — and a harness that *can* authenticate itself does not need to be wrapped.
...
- **AGEN-05**: shipped 2026-07-30. Agent token/path detection reads `ignore.IsSecretName` plus canonical `CredentialHomeDirs`/`CredentialHomeFiles`; the sandbox reads the same home lists. `.env` template exceptions now agree across scan, draftbundle, and the wrapper; the redundant `cat .env` substring pattern was deleted so the argv-aware file policy owns that decision. `*.key` remains an explicitly documented agent-only guardrail.
