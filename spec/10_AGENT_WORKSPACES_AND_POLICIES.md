...
- an OS-enforced sandbox (macOS Seatbelt, Linux bubblewrap → Landlock+seccomp fallback) now confines every non-`yolo-local` run beneath these guardrails; containerization stays the later slice.
...
5. an OS-enforced sandbox (Seatbelt / bubblewrap+Landlock+seccomp) shipped 2026-07-05 (`P4-GIT-03`); containerization stays the later slice.
