...
- **OS-enforced sandbox: macOS Seatbelt and Linux bubblewrap shipped** (`AGEN-03`/`P4-GIT-03`, 2026-07-05). The wraps above are kernel-enforced for the child and everything it spawns — the interpreter bypasses in the first bullet can no longer write outside the worktree, read/mask the denied credential paths, or (under `readonly`/`cautious`) touch ordinary network sockets. Remaining follow-ups: Landlock layered fallback, seccomp, and `sandbox.violation` telemetry. Linux caveats: credential masks hide paths (ENOENT/empty) rather than returning EPERM, and nested-sandbox tools (Chrome headless, flatpak, bwrap-in-bwrap) can break under `--disable-userns`; `--sandbox off` is the explicit escape hatch.
