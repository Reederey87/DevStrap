...
> **Zero-infrastructure carrier (`AD-1`) — first slice shipped 2026-07-04.** Because the hub only ever holds ciphertext plus signed events, a dumb carrier is a safe zero-knowledge boundary: the **private-git-repo backend** (`hub: git+ssh://…`, behind the same pluggable `Hub` interface) removes the "provision an R2 bucket first" adoption friction using credentials the user already has. The merge-conflict objection above does not apply to the carrier design: every object key is content-addressed or `(device,seq)`-unique, so no `git merge` ever runs — a lost push race refetches and re-applies. Remaining direction: make it the quickstart default (keeping `r2://` as the scale/power option) and add a local-folder/cloud-drive-folder carrier; see `03_SYSTEM_ARCHITECTURE.md` (Hub backends) and `14_MVP_ROADMAP_AND_BACKLOG.md`.
