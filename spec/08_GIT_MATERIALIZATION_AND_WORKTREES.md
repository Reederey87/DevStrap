...
- **A sparse failure never fails materialization.** If `SparseCheckoutInit`/`SparseCheckoutSet` fail, DevStrap disables sparse-checkout and checks out the FULL tree, degrading to exactly the pre-`W14-01` outcome rather than abandoning the clone. Only a failure of the checkout itself (an empty working tree) is fatal.
