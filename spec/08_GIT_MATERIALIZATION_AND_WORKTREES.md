...
**A typo'd directory is warned about, never refused (`W14-02`).** `ValidSparsePath` is purely *syntactic*, and `git sparse-checkout set` does not error on a directory that does not exist — cone mode simply narrows to nothing under it. So `devstrap add --sparse bakend` (for `backend`) used to succeed, report success, and leave a working tree of top-level files only, with no signal anywhere. `Runner.MissingSparseDirs` now probes the configured set against the tree at `HEAD` and every unmatched path is named in a warning carrying both remedies (`project sparse set` to correct, `project sparse clear` to restore the full tree).

Four properties of that probe are deliberate:
...
The probe normalizes its input (`NormalizeSparsePaths`) rather than trusting callers to, for the same reason `ApplyConvergedSparseCheckout` does: `ls-tree` collapses an ancestor/descendant pathspec pair down to the descendant, so an overlapping set would report the *ancestor* as missing — a false warning about a valid profile, the exact failure the probe is written to avoid.

**Checked at apply time, not at `add` time**, which is the load-bearing scoping decision: at `devstrap add --sparse ...` there is usually no checkout yet, so an add-time probe would silently never run while looking like it did.
...
`devstrap worktree new --fresh-upstream` applies the SAME project's sparse profile to the fresh worktree it creates, immediately after `git worktree add` succeeds — this is the step most likely to be silently missed, since skipping it would defeat the whole feature for agent worktrees on a large monorepo. `devstrap worktree adopt` (registering a worktree an external harness such as Codex/Cursor/Devin already created) deliberately does **not** apply a configured profile to the adopted checkout — adopt never mutates a checkout it did not create — and instead surfaces a warning naming the manual `git sparse-checkout init --cone && git sparse-checkout set` remedy. **That warning states what DevStrap did, not what the tree looks like (`W14-06`).** It previously said the profile "was NOT applied to this adopted worktree", which reads as a claim about the resulting working tree — and adopt cannot know that: a real `git worktree add` against an already-narrowed repo **inherits the active cone** from git's own shared repo-level `core.sparseCheckout` config, with no sparse-checkout command run against the new worktree at all (empirically observed while writing `W12-02`'s own adopt test, which had to drop exactly that assertion). "DevStrap issued no sparse-checkout command" is knowable and is what the warning now says; whether the checkout ended up narrowed is git's behavior to determine, and the warning no longer pretends otherwise. The inconsistency remains explained rather than silent — it is simply explained in terms of the action taken instead of an outcome asserted.
