...
**Watch state and degrade path (`PLAT-02`, `M5D-04`).** `/v1/health` reports the watch plane as an explicit state: `starting` before roots have been resolved even once, `idle` for the entirely normal fresh-workspace state in which no projects are materialized yet, `watching` when the native watcher is armed, and `degraded` when the native watcher failed and the plane is polling or has no fallback. `starting` earns its place rather than letting the field default to the empty string: the zero value would reintroduce on the wire exactly the ambiguity this field removes — a consumer could not distinguish "not started yet" from "never reported" — and the window is real, since resolving roots opens SQLite. The compatibility `degraded` boolean is derived from that state. `state`, `degraded`, and `roots` are always present on the wire — including `false` and `0` in the idle case — while `backend`, `reason`, and `hints` remain conditional. The idle reason is `no materialized projects yet`; it is status, not an alarm.
