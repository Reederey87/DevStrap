...
<<<<<<< HEAD
- `restore [--force] [--allow-legacy] <archive>` (`P7-DATA-04`) stages extraction, rejects unsafe/non-regular entries, verifies manifest format/version, every entry hash/size, required-set membership, and absence of extra files, then read-only validates SQLite and cross-checks content-addressed blobs, current-device keys, and held-WCK files against the staged DB before any live swap. Pre-manifest archives require `--allow-legacy`, which warns and skips only manifest integrity. Verified targets retain the existing sequential `.bak` sibling promotion with per-target rollback; un-captured state-dir contents remain intact. A keychain-custody restore prints file-custody reconciliation guidance, and `--json` carries all warnings only in the single result document;
<<<<<<< HEAD
=======
=======
- `backup --full` (`P6-DATA-04`/`P7-DATA-03`) writes a single `tar` archive of the SQLite database (via `VACUUM INTO`), `config.yaml` (the `hub:` pointer + custom `root:`, when present), every referenced `age`-encrypted blob (`blobs/<sha256>.age`), and the device/workspace key material (`keys/…` — captured verbatim from the KeyDir under file custody, or escrowed out of the OS keychain under keychain custody). Blob refs are enumerated from the `VACUUM INTO` snapshot (not the live store), with a bounded retry when a concurrent rotation/GC leaves a superseded ciphertext missing; a still-missing referenced blob is **fatal** (exit conflict; run `devstrap doctor`) rather than warn-and-skip. Key material still stages from the live store (append-mostly custody data; snapshot purity applies to blob refs). If a required key file (device age/signing) or keychain-held secret cannot be read the backup fails loudly rather than producing a silently-incomplete "full" archive. Every archive entry and the output file are mode `0600`. This is the only backup that can recover captured secrets — a DB-only backup restores the database but leaves every `age_blob:` ref dangling and drops the hub pointer. Under `--json` (`P7-CLI-01`), warnings (no key material, no `config.yaml`) are carried in the payload's `warnings` array and never precede the JSON document;
- `restore` (`P6-DATA-04`) extracts a `--full` archive and replaces only the captured paths (`state.db`, `config.yaml`, `blobs/`, `keys/`) **in place**, leaving un-captured state-dir contents (e.g. `quarantine/`, `logs/`) intact. It refuses when a captured target already exists unless `--force`. Extraction is staged in a sibling temp dir, guarded against path traversal (zip-slip: absolute paths and `..` components are rejected, entries are confined to the `state.db`/`config.yaml`/`blobs/`/`keys/` layout), the staged DB is validated (`quick_check` + `foreign_key_check`) before any swap, and each captured path is swapped in via a `.bak` sibling + rename with rollback (`0600`); a restore of a keychain-custody DB prints the file-custody reconciliation guidance. Under `--json` (`P7-CLI-01`), that guidance (and any other warnings) is carried in the payload's `warnings` array and never precedes the JSON document;
>>>>>>> 2cda99e (fix(cli): db backup --full enumerates blob refs from the snapshot; missing blob fatal (P7-DATA-03))
=======
- `backup --full` (`P6-DATA-04`, hardened by `P7-DATA-03/04`) writes a single mode-`0600` tar. The accepted `VACUUM INTO` snapshot is opened read-only and supplies blob refs, custody, device/workspace identity, and held WCK epochs; the existing bounded retry handles concurrent blob rotation/GC. Required key bytes resolve from the live custody backend using that snapshot inventory. Missing/unreadable/content-address-mismatched blobs or required keys are fatal and the partial archive is removed. A final `manifest.json` v1 records size/SHA-256 and required-set membership for every earlier entry. Under `--json` (`P7-CLI-01`), non-fatal warnings stay in the payload's `warnings` array;
- `restore [--force] [--allow-legacy] <archive>` (`P7-DATA-04`) stages extraction, rejects unsafe/non-regular entries, verifies manifest format/version, every entry hash/size, required-set membership, and absence of extra files, then read-only validates SQLite and cross-checks content-addressed blobs, current-device keys, and held-WCK files against the staged DB before any live swap. Pre-manifest archives require `--allow-legacy`, which warns and skips only manifest integrity. Verified targets retain the existing sequential `.bak` sibling promotion with per-target rollback; un-captured state-dir contents remain intact. A keychain-custody restore prints file-custody reconciliation guidance, and `--json` carries all warnings only in the single result document;
>>>>>>> f95254c (fix(cli): versioned backup manifest + fail-closed restore verification (P7-DATA-04))
=======
...
>>>>>>> 899296c (fix(cli): journaled all-or-nothing restore promotion + maintenance lock (P7-DATA-05))
>>>>>>> 69a7b1c (fix(cli): journaled all-or-nothing restore promotion + maintenance lock (P7-DATA-05))
