...
- `daemon status` exits 0 whatever the run state — the same contract `service status` has — and honors `--json` through the `Renderer` seam: `{socket, running, pid, version, uptime, detail}`. The **socket is the source of truth**: a daemon that answers is running whatever the pid file says; the pid file only enriches the report.
...
**Maintenance-lock interop.** The daemon does **not** hold the shared state-home maintenance lock for its lifetime — as of this slice, nothing in the daemon takes it at all. Convergence, a later slice of this wave, **will** take it per tick and release it exactly as `run-loop` does, so that `db backup --full`, `db restore`, and `db down` keep working against a running daemon. Stated in the future tense deliberately: that behavior is a property of a slice that is not yet written, not shipped-and-verified behavior of this one.
