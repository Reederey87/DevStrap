...
[ ] SSO
[ ] audit logs
[~] containerized agent sandbox (macOS Seatbelt + Linux bubblewrap slices shipped 2026-07-05, P4-GIT-03; Landlock/seccomp + containerization still open)
...
- **Agent wrapper policy is still not the sandbox** (`AGEN-01`, `AGEN-03`): argv-substring policy is bypassable by any interpreter. Credential env stripping (`AGEN-02`/`SECU-02`) is shipped, and macOS Seatbelt + Linux bubblewrap now provide default-on OS enforcement where available; remaining hardening is project/env allowlists plus Landlock/seccomp/telemetry follow-up slices.
