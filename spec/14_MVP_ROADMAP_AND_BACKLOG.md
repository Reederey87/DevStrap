...
2. **Cost is paid per tick, not per change.** Each tick unconditionally walks `~/Code` (`scan.Walk`), pulls the hub, and runs a materialize pass — whether or not anything changed. Shortening the interval to buy latency multiplies filesystem walks and hub round-trips across the whole fleet (R2 bills per request; the `git+ssh://` carrier pays a fetch per tick). Watcher hints invert the trade: **converge when something actually changed**, so the fleet can hold a long, cheap safe-interval *and* low latency at the same time. This is the single strongest argument for the daemon, and note what it argues for — a *trigger*, not a new engine.
   ...
   > **Corrected 2026-07-31 (`P9-DAEMON-02`).** This argument was sound but the shipped code did not honour it: `runLoopTick` ran its full-workspace `scan.Walk` — two git subprocesses per discovered repo — *unconditionally*, including for a watcher-hint-triggered cycle. The "cheap enough to run every tick" judgement it relied on (`P6-XP-03`/`P6-XP-05`) was made against run-loop's **5-minute** default; the watcher's trigger floor is **5 seconds**, 60x more frequent. So a hint-driven convergence was scoped identically to the periodic tick it was meant to be cheaper than, and the trade never actually inverted. Hint-triggered cycles now skip scan+adopt; the next periodic tick still adopts newly-created local projects, which is correct precisely because the watcher is a latency optimization that makes no completeness claim.
