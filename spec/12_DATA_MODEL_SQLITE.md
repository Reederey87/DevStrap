...
| `blobs/<sha256>.age`      | Every `age`-encrypted blob the **snapshot** DB references (env `secret_bindings` + `draft_snapshots`), enumerated from the `VACUUM INTO` file with bounded retry (`P7-DATA-03`). A still-missing referenced blob is fatal (not warn-and-skip). Key material still stages from the live store. |
