...
| `manifest.json`           | Final tar entry: format `devstrap-full-backup`, version `1`, RFC3339 creation time, snapshot workspace/device/custody identity, and `{name,size,sha256}` plus `required` for every non-manifest entry. |
