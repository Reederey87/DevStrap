...
**Tombstone GC never purges a `namespace_entries` row that still has a `worktrees` row** (`P8-SEC-01`, 2026-07-31). `worktrees.namespace_id` is `ON DELETE CASCADE`, so before this guard a routine `devstrap hub compact` — `--gc-tombstones` defaults to **true** — silently erased live worktree registrations, including dirty adopted ones with uncommitted work, and left orphaned `agent_runs` rows behind via their `ON DELETE SET NULL` FK. The checkout and its diff survive on disk; DevStrap's record of them does not, so the stale-base gate and `agent pr`'s base provenance stop applying. The delete-vs-dirty guard on the sync side does not help, because it inspects only the MAIN checkout's dirty state — a dirty *linked* worktree never blocks the tombstone. Retaining a tombstone is cheap and bounded by devices × projects, and the next GC reclaims it once the worktree is finalized or removed; losing a registration is not recoverable.

`branch` is `""` for a worktree adopted while on a detached HEAD (the common shape for a Codex/Devin worktree). That column is **refreshed on re-adoption** and by `agent pr` when it finds a live branch (`P8-ADOPT-02`, 2026-07-31) — before that it was written once at insert and never updated, which made `agent pr`'s own printed remedy ("create a branch, then re-run") impossible to act on. `created_by` is `"agent"` for a worktree `devstrap worktree new --fresh-upstream` created, or `"adopted"` for one `devstrap worktree adopt` registered after the fact (AD5-02) — an externally-created linked worktree (Codex/Cursor/Devin) that DevStrap did not provision itself. `idx_worktrees_active_path` (migration `00032_worktrees_active_path_unique.sql`), a `UNIQUE` index on `(namespace_id, path) WHERE status = 'active'`, enforces at most one active row per physical worktree regardless of which command registered it; a removed worktree's path remains reusable since the predicate only scopes active rows. Because the index is keyed on the path STRING, rows written before `00032` — all of them by `worktree new`, since `EvalSymlinks` arrived with `adopt` itself — store an unresolved spelling the resolved lookup cannot alias-match; `adopt` retries with the unresolved spelling and canonicalizes such a row's **path only**, never its base (`P8-ADOPT-07`, 2026-07-31). The retry matches string-equal spellings only, which is stated rather than implied: an `os.SameFile` sweep would be complete, and is deliberately not built for a P3 that self-heals via cleanup's path-missing prune.
...
Current implementation writes `agent_runs` for the thin generic runner. Runs record the associated worktree, engine, task, base ref/SHA, branch, `0600` log path, diff summary, test/command summary, recorder PID and opaque same-boot process start-time identity (`runner_started_at`, migration 00024), sandbox backend/mode/limitations, and status (`running`, `complete`, `failed`, or `interrupted`). The PID/start-time pair prevents a recycled PID from keeping a crashed run `running`; legacy NULL/zero identities retain PID-only behavior (`P7-GIT-03`). The sandbox fields are unsigned local visibility: `sandbox_backend` is the selected OS backend (`seatbelt`, `bwrap`, `landlock`, or empty when unconfined), `sandbox_mode` records the requested `--sandbox` mode even when `auto` degrades to advisory-only, and `sandbox_limitations` stores a JSON array string for reduced-guarantee backends (empty when none).
...
CREATE TABLE sync_chain_anchors (
  ...
  folded_hash TEXT NOT NULL DEFAULT '',   -- 00028: P4-SYNC-05 fold seed at anchor_seq
...
`sync_chain_anchors` (migration `00020_sync_chain_anchors.sql`, `P4-SYNC-02`) records, per origin device, the content hash of the LAST event a snapshot covers (at `anchor_seq = floor-1`). A snapshot-bootstrapped device holds no event rows below the retention floor, so `previousEventContentHash` (the prev-hash check in `validatePrevEventHash`) would otherwise fail the first post-floor event of every device forever; the anchor is that check's fallback predecessor. `Tx.UpsertChainAnchor` writes one row per device inside `ImportSnapshot`'s transaction and keeps the HIGHEST `anchor_seq` on conflict (a floor only ever moves forward, so a stale re-import must never lower an anchor). The lookup is by `(device_id, anchor_seq)` — the store enforces a singleton workspace. `snapshot_sha256` records which sealed object carried the anchor, for audit. The `folded_hash` column (`00028`, `P4-SYNC-05`) additionally carries the running FOLD at `anchor_seq` so a snapshot-bootstrapped device can re-seed the omission-detection fold at the floor (it holds no events below it); it is empty for pre-`P4-SYNC-05` anchors, which degrades that origin's omission check to fail-safe. No secret material is stored.

### device_heads (shipped — folded-hash omission-detection watermark, P4-SYNC-05)

```sql
CREATE TABLE device_heads (
  workspace_id TEXT NOT NULL,
  device_id TEXT NOT NULL,             -- the ORIGIN/peer device whose head we track
  promised_seq INTEGER NOT NULL,       -- highest signed-head seq verified from that peer's ack
  promised_fold TEXT NOT NULL,         -- the fold the peer committed to at promised_seq
  promised_hlc INTEGER NOT NULL,       -- ack ProducedAt when promised_seq was recorded
  omission_pending INTEGER NOT NULL DEFAULT 0, -- 1 = shortfall observed last cycle (one-cycle grace)
  updated_at TEXT NOT NULL,
  PRIMARY KEY (workspace_id, device_id)
);
```

`device_heads` (migration `00028_device_heads.sql`, `P4-SYNC-05`) is the pull-path omission detector's per-origin-device watermark. `VerifyPeerHeads` reads each approved peer's SIGNED head (its ack marker's `folded_hash` at `pushed_through_seq`, see `07`) and, per peer, records the HIGHEST promise it has verified. The promise is monotone — a hub serving a stale (lower-seq) ack cannot retract a promise already recorded — so a hub that WITHHOLDS a peer's newest events is caught: the promise commits to a seq beyond the contiguous prefix this device folded, and a shortfall that persists past the one-cycle in-flight grace (`omission_pending`) raises an `event_omission` conflict (`withheld_tail`). When this device DOES hold the promised prefix, its independently-recomputed fold must equal `promised_fold`; a mismatch is a `fork` (equivocation or a spliced stream). This is per-peer OBSERVATION state — not derivable from the event log, since it records what a peer *claimed* over time — so it is persisted rather than recomputed. No secret material is stored.

### device_gitstate (shipped — working-state validation plane Layer A mirror, P7-GITSTATE-01)

```sql
CREATE TABLE device_gitstate (
  workspace_id TEXT NOT NULL,
  device_id TEXT NOT NULL,             -- the OBSERVING device (event.device_id); no FK, see below
  path_key TEXT NOT NULL,              -- case-folded namespace path; no FK, see below
  path TEXT NOT NULL,                  -- display-form namespace path
  ...
  stash_count INTEGER NOT NULL DEFAULT 0,
  observed_at_hlc INTEGER NOT NULL,    -- the source event's own HLC
  source_event_id TEXT NOT NULL,
  updated_at TEXT NOT NULL,
...
`device_gitstate` (migration `00029_device_gitstate.sql`, `P7-GITSTATE-01`) is the apply-side mirror for `repo.gitstate.observed` (`07`, working-state validation plane Layer A — "did I forget to push, and is that visible from another machine"). Capture runs `git --no-optional-locks status --porcelain=v2 --branch`, which never writes `.git/index`, so it is safe to run against a repo another process is using; a resolved upstream SHA and the current stash count are folded in as two cheap follow-up calls. Apply is MIRROR-ONLY — `UpsertDeviceGitstateTx` `INSERT ... ON CONFLICT DO UPDATE`s the row for `(device_id, path_key)`, guarded by `WHERE excluded.observed_at_hlc >= device_gitstate.observed_at_hlc` so an out-of-order redelivery of a stale observation cannot regress a newer one — this table holds "current state as last observed," never a history log.

No FK to `devices`: mirrors `device_heads` (`00028`) — remote devices are not enrolled in the local device registry until Phase 2, so an FK would break the first time this device observes gitstate from a peer it has not locally enrolled yet.

No FK to `namespace_entries` either, unlike the env/draft pointer events: this plane does not need the observed project to exist locally before the mirror can apply (there is no `env_pending_project`-style quarantine class for `repo.gitstate.observed`) — `path`/`path_key` are stored verbatim as an opaque identifier, joined against `namespace_entries` only by callers that need to (the future `status --all-devices`/`doctor` CLI surfacing, `AGENTS.md`-tracked as a separate change). This plane is strictly separate from agent worktree-base resolution — `worktree new --fresh-upstream` always bases from `origin/<default_branch>`, never from anything recorded here. No secret material is stored.

### device_wip (shipped — working-state validation plane Layer B mirror foundation)
...
CREATE TABLE device_wip (
  ...
  path_key TEXT NOT NULL,              -- case-folded namespace path; no FK, see below
  path TEXT NOT NULL,                  -- display-form namespace path
  ref TEXT NOT NULL DEFAULT '',        -- refs/devstrap/wip/<device_id>/<path_key>
  sha TEXT NOT NULL DEFAULT '',        -- the pushed `git stash create` commit
  base_sha TEXT NOT NULL DEFAULT '',
  captured_at TEXT NOT NULL DEFAULT '',
  observed_at_hlc INTEGER NOT NULL,    -- the source event's own HLC
  source_event_id TEXT NOT NULL,
  updated_at TEXT NOT NULL,
  dropped_at_hlc INTEGER,              -- drop event HLC; NULL before any drop
  dropped_sha TEXT,                    -- exact SHA removed by the leased delete
  PRIMARY KEY (workspace_id, device_id, path_key)
);
```
...
### project_sparse_paths (shipped — cone-mode sparse-checkout profile, W12-02)
...
`project_sparse_paths` (migration `00033_project_sparse_paths.sql`, `W12-02`) records a project's opted-in git cone-mode sparse-checkout directories — one row per configured directory, keyed by `(namespace_id, path)` so the same directory cannot be recorded twice. This narrows the *working tree* of an already-materialized repo, complementing (not replacing) the existing blobless partial clone: partial clone reduces transfer cost, sparse-checkout reduces working-tree/index cost, and the two compose (see `08_GIT_MATERIALIZATION_AND_WORKTREES.md` for the full distinction against the `.devstrapignore` compiler, which controls sync/materialization inclusion universally, *before* anything is cloned).

Deliberately **local-only and never synced** through the signed event log: no `workspace_id` column (this table only needs to chain through `namespace_id -> namespace_entries -> workspaces`, matching `draft_snapshots`' pattern) and no `source_event_*` coordinates — nothing here is ever event-sourced or applied from a peer's event. Each device chooses its own sparse profile. This mirrors the documented intent of the pre-existing (and still unused) `git_repos.sparse_config TEXT` column from `00001_initial.sql` — internal/sync's snapshot payload comment already states "device re-derives sparse config on materialization" — and `project_sparse_paths` is the table that actually implements that intent; the unused column is left in place rather than dropped, since dropping a column shipped in the initial schema is a separate, higher-risk migration this change does not need to take on.

`internal/git.Runner.SparseCheckoutInit`/`SparseCheckoutSet`/`SparseCheckoutList`/`SparseCheckoutDisable` are the git primitives (cone mode only — non-cone/pattern mode is rejected by `ValidSparsePath`, since git's own documentation deprecates it: O(N×M) pattern-matching cost, and it is incompatible with `--sparse-index`). `Runner.ApplyConvergedSparseCheckout` composes them: it validates every requested path before any git mutation runs, reads the currently active cone set and no-ops when it already matches the desired set (so an idle `devstrap sync` on a project with a configured profile stays a zero-subprocess-churn no-op, matching this codebase's existing idle-sync-is-a-no-op convention for `device_gitstate` capture), and on a genuine `SparseCheckoutSet` failure after a successful `SparseCheckoutInit` restores the prior active cone rather than unconditionally disabling sparse-checkout — falling back to a full checkout would be a worse outcome than a repo that already had a working profile before the call (see `08_GIT_MATERIALIZATION_AND_WORKTREES.md` § *Sparse checkout* for the full Codex-review rationale). Applying the profile is wired into both success paths of `hydrateProjectUnlocked` (the shared clone/materialize function `devstrap sync`/`hydrate`/`worktree new` all call) so re-running sync after a profile is configured or changed converges the tree BIDIRECTIONALLY — an empty configured set also disables a leftover on-disk cone, self-healing a `project sparse clear` whose own immediate disable call failed — and is always best-effort: a git failure here warns and records via `RecordProjectWarning` rather than failing materialization, since narrowing the working tree is a disk/index-cost optimization, not a correctness requirement. `devstrap worktree new` applies the SAME project's profile to the fresh worktree it creates immediately after `git worktree add` succeeds; `devstrap worktree adopt` deliberately does **not** apply it to an externally-created worktree it did not create — instead it surfaces a warning naming the manual `git sparse-checkout` remedy, so a configured-but-unapplied profile on an adopted worktree is an explained inconsistency rather than a silent one. `devstrap add --sparse dir1,dir2` persists the initial profile in the same transaction as the project's `project.added` event (`Tx.SetSparsePathsTx`); `devstrap project sparse set/list/clear` change, read, and remove the profile on an already-adopted project. `set` acquires the project's repo operation lock ONCE, before its DB write, and holds it across both the write and the immediate re-apply (the first-cut design acquired the lock only inside a later helper, which let a concurrent `set`/`clear`/sync interleave DB writes and git applies out of order); `clear` disables the on-disk cone under the lock first, then always clears the DB regardless of outcome, relying on the bidirectional self-heal above rather than failing the command on a disable error.
...
-- Two index shapes on the same blob-reference columns, serving two different query shapes (P7-DATA-06):
CREATE INDEX idx_secret_bindings_encrypted_value_ref ON secret_bindings(encrypted_value_ref COLLATE NOCASE);           -- 00025 (LIKE-prefix enumeration)
CREATE INDEX idx_draft_snapshots_blob_ref ON draft_snapshots(blob_ref COLLATE NOCASE);                                -- 00025 (LIKE-prefix enumeration)
CREATE INDEX idx_secret_bindings_env_profile_ref
  ON secret_bindings(encrypted_value_ref, env_profile_id) WHERE encrypted_value_ref IS NOT NULL;                       -- 00026 (BINARY exact-match rewrap)
CREATE INDEX idx_draft_snapshots_namespace_ref ON draft_snapshots(blob_ref, namespace_id);                            -- 00026 (BINARY exact-match rewrap)
```

The revoke/rewrap path touches these two blob-reference columns with **two different query shapes**, which is why there are two index shapes on each. The 00025 `COLLATE NOCASE` indexes serve the *enumeration* scans (`AllBlobRefs`/`EnvBlobRefs`/`DraftBlobRefs`/`RetainedBlobRefs`), which filter with `col LIKE 'age_blob:%'` — `NOCASE` is required because that is the collation SQLite's default case-insensitive `LIKE` optimization needs to engage an index. The 00026 `BINARY` composite indexes serve the *exact-match* per-ref lookups the audit finding is actually about — `EnvProfilesForBlobRef` (`b.encrypted_value_ref = ?`), `DraftSnapshotsForBlobRef` (`ds.blob_ref = ?`), and the `UpdateBlobRef` UPDATEs (`WHERE encrypted_value_ref = ?` / `WHERE blob_ref = ?`), each run **per distinct ref** by `blob_gc.go`'s rewrap loop. A NOCASE index cannot serve a BINARY-collation `= ?` comparison, so without the composite indexes those per-ref lookups full-scanned the single writer — toward quadratic on large fleets. The leading column is the equality column; `secret_bindings` is partial on non-NULL refs because provider (`op://`) bindings the rewrap loop never touches carry a NULL `encrypted_value_ref`. `EXPLAIN QUERY PLAN` regression tests (`TestBlobRefRewrapUsesCompositeIndexes`) pin that each exact-match query uses its composite index — e.g. the `secret_bindings` join in `EnvProfilesForBlobRef` becomes `SEARCH b USING COVERING INDEX idx_secret_bindings_env_profile_ref (encrypted_value_ref=? AND env_profile_id=?)`.
...
internal/state/migrations/
  ...
  00024_agent_run_runner_started_at.sql
```
...
internal/state/migrations/00024_agent_run_runner_started_at.sql
internal/state/migrations/00025_blob_ref_indexes.sql
internal/state/migrations/00026_blob_ref_composite_indexes.sql
internal/state/migrations/00027_device_revoked_at_hlc.sql
internal/state/migrations/00028_device_heads.sql
internal/state/migrations/00029_device_gitstate.sql
internal/state/migrations/00030_device_wip.sql
internal/state/migrations/00031_device_wip_dropped_at.sql
internal/state/migrations/00032_worktrees_active_path_unique.sql
internal/state/migrations/00033_project_sparse_paths.sql
```

The current schema version is **33**. `00010_repo_forge_kind.sql` adds the per-project forge override (`GIT-05`); `00011_pending_hub_deletes.sql` queues blobs orphaned by a local-only revoke for deletion on the next hub-enabled sync (`P5-PROD-02`/`P5-SEC-01`); `00012_draft_snapshot_idempotency.sql` adds a partial `UNIQUE` index on `draft_snapshots(namespace_id, source_event_id)` so idempotency is enforced by the DB, not only the SELECT-then-INSERT guard (`P5-DATA-02`); `00013_workspace_keys.sql` adds the `workspace_keys` and `workspace_key_grants` tables backing the WCK epoch keyring for envelope encryption of the event log (`P4-SEC-02`/`P4-SEC-07`) — `workspace_keys(workspace_id, epoch, created_at)` records which epochs this device holds, and `workspace_key_grants(workspace_id, epoch, recipient, source_event_id, source_event_hlc, source_event_device_id, created_at)` is a membership audit of device.key.granted events (the wrapped WCK itself rides the event payload, never SQLite); `00014_workspace_key_kids.sql` re-keys `workspace_keys` by `(workspace_id, epoch, kid)` and adds `origin` (`P6-SEC-02`/`P6-SEC-01b` — same-epoch keys coexist under content-derived kids instead of overwriting; pre-kid rows backfill as `kid=''`/`origin='legacy'`) and adds the nullable audit `kid` column to `workspace_key_grants`; `00015_key_grant_waits.sql` adds the `key_grant_waits` grace-window table for never-granted workspace keys (`P6-SEC-03`, see its section above); `00016_device_hlc_index_single_local.sql` adds `idx_events_device_hlc` for device-scoped HLC event scans (`P6-DATA-05`) and `idx_devices_single_local` to enforce exactly one local device row (`P6-DATA-06`); `00017_hub_device_cursors.sql` adds the per-origin-device Seq transport cursor table (`P5-SYNC-01`, see its section above) and freezes `hub_cursors` as a read-only legacy row set; `00018_sync_skipped_events.sql` adds the durable pull-drop record (`P6-SYNC-02`, see its section below); `00019_local_meta.sql` adds the `local_meta(key, value, updated_at)` key/value table for machine-local, never-synced decisions — its first consumer is `key_custody` (`keychain` or `file`), the device/workspace secret-custody backend recorded once at init from a keychain reachability probe and honored on every later run so a store never silently migrates backends (`P6-XP-04`; the split-custody wedge that let a headless run mint a divergent signing key). `local_meta` holds no secret material and is intentionally not workspace-scoped — it describes the host, not the synced namespace; `00020_sync_chain_anchors.sql` adds the `sync_chain_anchors(workspace_id, device_id, anchor_seq, anchor_content_hash, anchor_hlc, snapshot_sha256, imported_at)` table backing per-device hash-chain anchors imported from a snapshot (`P4-SYNC-02`, see its section below) — a snapshot-bootstrapped device has no event rows below the retention floor, so the prev-hash verification of the first post-floor event per origin device falls back to its anchor (the content hash of the last covered event, at `seq = floor-1`); `00021_agent_run_runner_pid.sql` adds nullable `agent_runs.runner_pid` so the CLI can distinguish a genuinely running agent recorder from a crash-stuck `running` row and reconcile dead processes to `interrupted` (`P6-GIT-06`) — and `worktree cleanup` refuses to reap any worktree with a still-running `agent_runs` row via the by-worktree query `RunningAgentRunsByWorktree` (`P7-GIT-01`); `00022_sandbox_telemetry.sql` adds `agent_runs.sandbox_backend`, `sandbox_mode`, `sandbox_limitations`, and the local `sandbox_violations` table for macOS Seatbelt denial visibility (`P4-GIT-03` slice 5); `00023_env_profile_source_events.sql` adds env profile source-event coordinates for `env.profile.updated` LWW/idempotency (`ENV-SYNC-01`); `00024_agent_run_runner_started_at.sql` adds nullable `agent_runs.runner_started_at`, the opaque start-time half of the PID-reuse guard used by stale agent-run reconciliation (`P7-GIT-03`); `00025_blob_ref_indexes.sql` adds `COLLATE NOCASE` indexes on `secret_bindings(encrypted_value_ref)` and `draft_snapshots(blob_ref)` so the revoke/rewrap blob-reference scans (`AllBlobRefs`/`EnvBlobRefs`/`DraftBlobRefs`/`RetainedBlobRefs`) use an index instead of full-scanning — the `NOCASE` collation is required for SQLite's default case-insensitive `LIKE 'age_blob:%'` optimization to engage the index (`P7-DATA-06` enumeration scans); `00026_blob_ref_composite_indexes.sql` adds the `BINARY` composite indexes `idx_secret_bindings_env_profile_ref` (`secret_bindings(encrypted_value_ref, env_profile_id)`, partial on non-NULL refs) and `idx_draft_snapshots_namespace_ref` (`draft_snapshots(blob_ref, namespace_id)`) that serve the exact-match (`= ?`) per-ref lookups the finding is actually about — `EnvProfilesForBlobRef`, `DraftSnapshotsForBlobRef`, and the `UpdateBlobRef` UPDATEs run per distinct ref by `blob_gc.go` — which a NOCASE index cannot serve; pinned by `EXPLAIN QUERY PLAN` regression tests (`P7-DATA-06` rewrap lookups; see the index section above); `00027_device_revoked_at_hlc.sql` adds the nullable `devices.revoked_at_hlc` column recording the HLC of the earliest revocation that flipped a device to a terminal state, so the apply path can TIME-SCOPE trust checks (`P7-SYNC-02`): a now-revoked device's pre-revocation content event (HLC strictly below the boundary) is admitted regardless of delivery order, while any event at or after the boundary stays rejected — NULL means the boundary is unknown and the device fails closed; `00028_device_heads.sql` adds the `device_heads` table backing the folded-hash-chain omission detector and a `folded_hash` column to `sync_chain_anchors` (`P4-SYNC-05`, see the `device_heads` section below); `00029_device_gitstate.sql` adds the `device_gitstate` table backing the working-state validation plane Layer A mirror for `repo.gitstate.observed` (`P7-GITSTATE-01`, see the `device_gitstate` section above); `00030_device_wip.sql` adds the `device_wip` table backing the working-state validation plane Layer B mirror foundation for `repo.wip.pushed` (see the `device_wip` section above); `00031_device_wip_dropped_at.sql` adds the nullable `device_wip.dropped_at_hlc`/`dropped_sha` pair backing `repo.wip.dropped` tombstones (`P7-WIP-06`) — the two move together on apply so the row can never hold a coordinate/sha combination no drop event carried, an empty `dropped_sha` marks a sha-agnostic "observed absent" drop that leased nothing, and the Down migration DELETEs dead rows before rebuilding because schema 30 encodes "dropped" as row absence; `00032_worktrees_active_path_unique.sql` adds `idx_worktrees_active_path`, a `UNIQUE` index on `worktrees(namespace_id, path) WHERE status = 'active'`, so the same physical worktree can never be registered twice while active (AD5-02, `devstrap worktree adopt`) — the `status = 'active'` scope deliberately lets a removed worktree's path be re-registered later, since a removed row's path is legitimately reusable; `00033_project_sparse_paths.sql` adds the `project_sparse_paths` table backing local-only, per-device cone-mode git sparse-checkout profiles (`W12-02`, see its section above). Migrations can be applied by `devstrap init` or explicitly with `devstrap db migrate`.
...
| `blobs/<sha256>.age`      | Every `age`-encrypted blob the snapshot DB references (env `secret_bindings` + `draft_snapshots`), enumerated with bounded retry (`P7-DATA-03`). Missing, unreadable, or content-address-mismatched ciphertext is fatal. |
...
| `manifest.json`           | Final tar entry: format `devstrap-full-backup`, version `1`, RFC3339 creation time, snapshot workspace/device/custody identity, a `{name,size,sha256}` entry for every non-manifest file, and a separate `required` LIST — the independently-defined recoverable core (`state.db` plus the device age and signing key files), never a mirror of the entry list — whose members must all be listed and staged for a restore to proceed. |

The `VACUUM INTO` file is authoritative: `state.OpenSnapshot` opens it read-only (`mode=ro`, `query_only`, no migration/chmod/WAL side files), and blob refs, custody, current device/workspace identity, and held WCK epochs are read from that frozen row-set. The existing bounded retry remains when a snapshot references ciphertext concurrently superseded by rotation/GC.

**Key capture is custody-aware and snapshot-selected.** In both file and keychain custody, the snapshot's device/workspace identity and exact held-WCK epoch inventory select bytes from the live custody backend through `HybridStore.ExportForBackup`; those bytes are escrowed into the archive's file-store layout. If a required secret cannot be read, the backup fails loudly rather than producing an incomplete "full" archive.
