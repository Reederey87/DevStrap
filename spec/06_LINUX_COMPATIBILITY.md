...
CI has a separate advisory `windows-latest` smoke job that builds and vets `./...`, then tests the platform-safe `pathkey`, `ignore`, `draftbundle`, `envfile`, `redact`, `id`, and `pairing` packages plus only this PR's own tests within `internal/platform` (`TestProcessAlive*`, `TestRuntimeGOOSBranchesStayInPlatformPackage`). `internal/git` is excluded entirely: its test harness fakes a `git` executable via a Unix-style `PATH` prepend that does not resolve on Windows — a test-fixture gap, not a production-code bug. The rest of `internal/platform` is deliberately NOT run wholesale: most of its pure-logic golden tests (bwrap/Seatbelt credential deny-lists, launchd/systemd unit rendering) hardcode Unix-style absolute paths and fail on Windows — a pre-existing cross-platform-test gap, confirmed on this job's first real runs and tracked as a follow-up, unrelated to the `ProcessAlive` fix this job exists to smoke-test. The job also deliberately excludes the broader CLI/hub/state suites and remains `continue-on-error` for its first cycle; this is build/vet/narrow-test visibility, not a claim of full Windows support.
