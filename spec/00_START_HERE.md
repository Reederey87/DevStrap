...
- Working-state validation plane Layer A (`P7-GITSTATE-01`): `repo.gitstate.observed` capture (`git.Runner.CaptureGitstate`), its event constructor/apply handler, the mirror-only `device_gitstate` storage, and the CLI surfacing — `status --all-devices` renders every device's last-observed working-state per project, and `doctor` grades staleness — both always render an explicit row per project ("never synced" or "last seen N ago"), never a silent all-clear. Wiring capture into `devstrap sync`'s runtime flow remains a follow-up (see "Not implemented yet" below).
...
- cross-machine working-state sync — the git-state validation plane's Layer A (`P7-GITSTATE-01`) backend and CLI surfacing are shipped (see "now built" above); wiring `CaptureGitstate`/`NewGitstateEvent` into `devstrap sync`'s runtime flow remains a follow-up. WIP refs (Layer B, `refs/devstrap/wip/*`) remain genuinely unbuilt; the encrypted draft-bundle layer (Layer C) is shipped;
