...
Mac watcher: fsnotify/kqueue (native FSEvents measured against and deferred 2026-07-29 — see spec/05)
...
- CLI entrypoint: `cmd/devstrap`; the WIP recovery family includes
  `wip gc` for TTL-bounded self/revoked/lost recovery refs.
- Commands: `version`, `init`, `up`, `join`, `pair`, `scan`, `export`, `import`, `add`, `clone`, `hydrate`, `promote`, `open`, `project sparse set`, `project sparse list`, `project sparse clear`, `sync --hub-file`, `hub init`, `hub compact`, `hub gc`, `hub login`, `hub logout`, `hub migrate-events`, `keys rotate`, `materialize`, `draft snapshot create`, `run-loop`, `daemon start`, `daemon stop`, `daemon status`, `daemon sync`, `daemon events`, `service install`, `service uninstall`, `service status`, `worktree new`, `worktree adopt`, `worktree status`, `worktree finalize`, `worktree list`, `worktree remove`, `worktree cleanup`, `worktree unlock`, `wip push`, `wip fetch`, `wip status`, `wip show`, `wip apply`, `wip drop`, `wip gc`, `env capture`, `env hydrate`, `env bind`, `env rotate`, `env op list`, `env op set`, `run`, `agent run`, `agent adopt`, `agent finish`, `agent list`, `agent show`, `agent pr`, `devices enroll`, `devices list`, `devices approve`, `devices revoke`, `devices lost`, `devices rename`, `devices recipient`, `devices pairing-code`, `conflicts list`, `conflicts show`, `conflicts resolve`, `mcp serve`, `status` (`--watch`, `--prompt`), `shell-init`, `doctor` (`--remote`), `db migrate`, `db status`, `db backup` (`--full`), `db restore`, and `db down`.
...
- `devstrap promote <path>` (`NOVCS-03`) graduates a remote-less namespace entry: `--draft` turns a `plain_folder` into a `draft_project` (content then rides the shipped `draft snapshot create` bundle plane — no second bundling path), and `--git-remote <url>` turns a `local_git`, `plain_folder`, or `draft_project` into a `git_repo`. `local_git` is the primary case and its **existing history is pushed** — a `local_git` is a real repository the user never pushed, so `git init` is never run over one; only the non-git sources are initialized (with one initial commit of their existing content). The URL is validated by the same `git.CanonicalRemoteKey` helper `scan`/`add` use, the remote must be **empty** (a remote already holding refs is an `add` case), and the `git_repo` row and its `project.updated` event are written only **after** the push succeeds, so a failed push leaves the project at its original type and rolls the working tree back to its pre-command state. Promoting a project that already has a remote is refused naming `add`, and demotion is out of scope. See `07_NAMESPACE_AND_SYNC_MODEL.md` § *Promotion*.
...
- Fresh upstream worktree creation that holds the repo operation lock, fetches `origin/<default_branch>`, resolves base SHA from the remote ref, honors stored LFS policy with pull-or-warn behavior, records worktree metadata, exposes `worktree status` stale-base detection, and gates `worktree finalize` on the recorded base unless `--allow-stale-base` is explicit. `devstrap worktree adopt <path>` (AD5-02) registers a linked worktree an agent harness created itself (Codex/Cursor/Devin) instead of `devstrap worktree new`: `git.Runner.WorktreeIdentity` classifies the path (refusing a main checkout, a non-worktree, or an unborn HEAD, while adopting a detached HEAD as the common case), the recorded `base_sha` is a `git.Runner.MergeBase(HEAD, base_ref)` — never the base ref's raw tip — with a shallow clone refused unless `--allow-shallow` and two histories sharing no common ancestor refused with a `--base-ref` remedy, and `idx_worktrees_active_path` (a DB-level unique index, migration `00032`) keeps at most one active row per physical worktree. Re-adopting the same path is idempotent — a row adopt created is refreshed, and a row it did not create (a `worktree new` worktree) is reported and left untouched, since rewriting a base adopt did not record is exactly what the AD-5 invariant forbids. `worktree new` has no such idempotent path and needs none: it mints a fresh timestamped, randomly-suffixed directory every run and never re-registers an existing one, so the index is there to turn any duplicate registration into an error rather than a silent second row. Per-project, local-only, cone-mode git sparse-checkout profiles (`W12-02`, migration `00033`) let a project opt a subset of its working tree into materialization — complementing (not replacing) blobless partial clone — applied on both `hydrateProjectUnlocked` success paths and inherited by every fresh `worktree new` worktree (best-effort; `worktree adopt` deliberately does not apply it to an externally-created checkout); see `08_GIT_MATERIALIZATION_AND_WORKTREES.md` § *Sparse checkout* for the mechanism and its distinction from `.devstrapignore`.
- Thin generic agent runner that creates a fresh worktree, runs explicit argv commands with a sanitized no-secret default environment and wrapper-level command/file path policy, captures a `0600` log, records `agent_runs`, summarizes Git status/diff, and gates `agent pr` on the recorded base before pushing and creating a forge-aware PR/MR when the relevant CLI is available (`gh`/`glab`/`tea`) or printing a compare URL for unsupported forges; the `gh`/`glab`/`tea` invocation itself, self-hosted-override precedence, missing-CLI degradation, and failure wrapping are now exercised hermetically via PATH-shimmed forge-CLI stubs (`FORGE-05`, closing the last forge-hardening gap). Project-env allowlists (`P4-GIT-06`, `internal/agentsecrets`): a project-root `.devstrapagent.yml`, read from the agent's fresh worktree, opts `agent run` into exposing its captured `devstrap_encrypted` env profile — filtered to `agent_secrets.allow` minus `agent_secrets.deny` (deny wins) — into the child process; a project with no config file injects no captured secrets, matching the pre-existing default.
...
- Plain-text workspace manifest export/import (`AD-7`, `W13-02`): `devstrap export --manifest <path> [--pinned]` writes a `vcstool` `.repos` document — root key `repositories`, entries keyed by relative path, each `{type, url, version}` — so `vcs import` reconstructs the tree with a binary DevStrap does not control, which is what makes "recover without DevStrap" checkable rather than asserted. DevStrap's own metadata sits under one sibling `devstrap` key that vcstool ignores by construction, never interleaved into the entries it parses. **The interop claim is scoped in the emitted file's own header, not only in the spec:** it holds for `git_repo` rows only; `local_git`/`plain_folder`/`draft_project` have no `{url, version}` and are structurally invisible to vcstool, and encrypted env/draft content never travels with a plaintext manifest (the manifest names which projects *have* an env profile and never carries one). `--pinned` records resolved SHAs (mirroring `vcs export --exact`) and OMITS `version` where HEAD cannot be resolved rather than degrading back to a branch. `devstrap import --manifest <path>` is a registration plane only — it writes namespace rows plus `project.added` events and lets the existing `sync`/`materialize` pass clone, since a second cloning path is what `spec/13` already refused for `/v1/status` — is idempotent, never overwrites a differently-registered project, exits non-zero on a partial import, and also accepts a bare third-party `.repos` file. `../docs/recovery.md` is the user-facing guide.
...
- Working-state validation plane Layer A (`P7-GITSTATE-01`, fully wired): `repo.gitstate.observed` capture (`git.Runner.CaptureGitstate`), its event constructor/apply handler, the mirror-only `device_gitstate` storage, the CLI surfacing (`status --all-devices` renders every device's last-observed working-state per project, and `doctor` grades staleness — both always render an explicit row per project, "never synced" or "last seen N ago", never a silent all-clear), and the runtime wiring — `devstrap sync` captures every already-materialized `git_repo` project's working state once per cycle, mirrors it into this device's own `device_gitstate` row immediately (not just onto the outbound event so peers see it once they pull), and skips re-emitting an unchanged capture so an idle sync stays a zero-push no-op.
- Working-state validation plane Layer B (`P7-WIP-01`): `repo.wip.pushed` — `git.Runner.StashCreate`/`PushRef` capture the working tree's uncommitted state as a commit object (no worktree/index changes) and push it via a raw refspec to this device's own `refs/devstrap/wip/<device_id>/<path_key>` ref, mirrored into the local `device_wip` table in the same transaction as the event insert so the pushing device sees its own state immediately (the pull-apply path dedups a device's own re-delivered events). `wip push <project>` and `wip fetch <project> [--device <id>]` are the user-facing CLI; fetch (`git.Runner.FetchRef`) always derives the target ref canonically from device id + path_key — never from a stored/peer-supplied `Ref` string — and, absent `--device`, discovers candidate devices from the synced `device_wip` mirror rather than probing the network. Strictly separate from agent worktree-base resolution: this plane is never read by the fresh-worktree resolver. `wip status <project>` and `wip show <project> [--device <id>]` are the read/inspect half: `status` lists every device's pending WIP from the local `device_wip` mirror (a plain "No pending WIP" line for the zero case, never a warning — unlike Layer A, an absent WIP ref is the normal, healthy state, not an unproven one); `show` fetches one device's ref (auto-selected when exactly one is pending, otherwise requiring `--device` with a usage error naming the candidates) and displays its stat summary and full diff via `git diff <ref>^ <ref>` — diffing against the stash commit's own first parent rather than a plain `git show <ref>`, since a stash-create commit is a merge commit and `git show`'s default combined-diff format for merges is harder to read — without applying anything to the working tree or index. `status --all-devices` and `doctor` also surface pending WIP alongside the Layer A gitstate columns: `status --all-devices` adds a compact per-project pending-WIP line (absent entirely when there is none), and `doctor` warns only once a pending WIP ref is older than 48h (a shorter threshold than Layer A's gitstate staleness, since a stash is expected to be resolved within a session or two) — a project with no or only recent pending WIP renders no row at all. `wip apply <project> [--device <id>]` and `wip drop <project> [--device <id>]` are the mutating half: `apply` fetches the resolved device's ref and runs `git stash apply` directly against the working tree with no pre-emptive dirty-tree gate — a clean apply succeeds and leaves the WIP's changes unstaged, while a genuine content conflict is git's own per-file conflict detection refusing safely (either an outright abort with no changes, whose stderr is surfaced as-is, or standard `<<<<<<<`-style conflict markers left in place, detected via `git.Runner.DirtyState`'s `DirtyConflicted` and reported for manual resolution — devstrap never attempts automatic conflict resolution or an abort/reset); `drop` deletes the remote ref (`git.Runner.DeleteRef`, a raw `git push <remote> :<ref>`) and clears only the dropping device's own local `device_wip` mirror row — deleting an already-gone ref is not an error, and the drop is never propagated to other devices' mirrors (they learn the ref is gone only the next time they try to fetch it). Both commands resolve their target device via the same helper `wip show` uses, so an ambiguous multi-candidate selection with no `--device` is the identical usage error naming every candidate.
- Shell prompt integration (`W12-01`): `devstrap status --prompt` is a fast, local-only, single-line summary (`clean`, or space-joined `dirty:N wip:N conflicts:N` from already-synced local mirror state — no git shell-out, no network I/O, no sync trigger) for embedding in a shell prompt, and `devstrap shell-init {bash|zsh|fish}` (`internal/shellhook`) prints eval-able shell code that WRAPS the shell's existing prompt-hook mechanism (bash's `PROMPT_COMMAND` array, zsh's `precmd_functions` array, fish's `fish_prompt` event) instead of replacing it, guarding against the breakage class that broke Starship when direnv's tcsh hook overwrote `precmd`.
...
Not implemented yet (genuinely unbuilt — features that are partly shipped are listed under "now built" below, never here; and features the project has DECIDED not to build are not listed here at all, because a withdrawn path in a not-*yet* list is a promise the project has decided not to keep — the daemon job model and its six unbuilt socket endpoints were withdrawn on 2026-08-01, `W13-09`, and now live in `13_CLI_DAEMON_API.md` § *Withdrawn*):
...
- the FSEvents-specific Mac watcher, **measured 2026-07-29 and deferred on the evidence** rather than left open: after `M5D-06`'s compiler-driven pruning a 37,799-directory tree watches 5,639 directories for 38,095 descriptors — 15.5% of the 245,760 that machine's `kern.maxfilesperproc` allows — so kqueue is not the constraint and cgo-on-darwin is not worth its distribution cost. **The reading is a proxy:** `~/Code` was empty on the measured machine, so the tree was the maintainer's real working tree rather than a managed namespace, on one machine and one macOS version. `05_MAC_FIRST_IMPLEMENTATION.md` carries the method, the caveat in full, and the threshold that would flip the verdict (~20,000 watched directories). `/v1/health` now reports that quantity conditionally as `watch.watched_dirs`; `watch.roots` still counts projects. **The daemon itself, its local socket API, and the watcher wiring shipped 2026-07-25** (Milestone 5): `devstrap daemon start|stop|status|events` over a peer-credential-authenticated Unix socket, and `devstrap service install --daemon` supervises it under launchd/systemd. It stays an optimization, never a correctness dependency — every command works with no daemon, and `devstrap run-loop` remains the portable daemonless convergence path;
...
- `../docs/install.md`, `../docs/quickstart.md`, `../docs/self-hosting.md`, `../docs/agents.md`,
  `../docs/recovery.md` — task-oriented user
  ...
  testscripts, so the documented sequences cannot silently rot; and — `W13-02` — recovering a
  workspace, covering both the DevStrap path and the `vcstool` path that needs no DevStrap at
  all). `../docs/audits/` holds the audit archive and open backlog.
