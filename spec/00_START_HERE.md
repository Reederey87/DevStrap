...
- cross-machine working-state sync — the git-state validation plane's Layer A backend is now SHIPPED (`P7-GITSTATE-01`): `repo.gitstate.observed` capture (`git.Runner.CaptureGitstate`), its event constructor/apply handler, and the mirror-only `device_gitstate` storage; wiring capture into `devstrap sync` and the `status --all-devices`/`doctor` surfacing are a follow-up. WIP refs (Layer B, `refs/devstrap/wip/*`) remain genuinely unbuilt; the encrypted draft-bundle layer (Layer C) is shipped;
