...
- the daemon's **job model** and every socket endpoint beyond `health`/`version`/`status`/`events`/`sync`; and the FSEvents-specific Mac watcher, still gated on the descriptor measurement `05_MAC_FIRST_IMPLEMENTATION.md` names — now observable for the first time as `/v1/health`'s `watch.roots`, but not yet taken against a real `~/Code`. **The daemon itself, its local socket API, and the watcher wiring shipped 2026-07-25** (Milestone 5): `devstrap daemon start|stop|status|events` over a peer-credential-authenticated Unix socket, and `devstrap service install --daemon` supervises it under launchd/systemd. It stays an optimization, never a correctness dependency — every command works with no daemon, and `devstrap run-loop` remains the portable daemonless convergence path;
