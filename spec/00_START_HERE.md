...
- the `devstrap mcp serve` stdio MCP server (`AD5-07`) — deliberately deferred rather than unbuilt-by-omission: the MCP protocol had a breaking rewrite on 2026-07-28, and adding an external dependency to a signed-release binary is a supply-chain decision to take on its own evidence. The reference integration shipped **without** it (`AD5-04`, `../docs/agents.md`), because the plain `--json` shell-out already works in Claude Code, Cursor, and Codex with zero integration code;
