...
- **automatic** fleet-wide WIP-ref TTL/GC on the convergence path — `wip drop` now propagates fleet-wide (`P7-WIP-06`) and `devstrap wip gc` expires aged refs on demand (`P7-WIP-07`, self + revoked/lost owners at a 30-day TTL, with deletion authority resting on the remote's advertised sha, the ref object's own committer date, and a `--force-with-lease` pin rather than on mirror state). What remains unbuilt is running that sweep automatically from the sync cycle (`P7-WIP-08`); today it is an explicit command.
