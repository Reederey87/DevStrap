...
- Commands: `version`, `init`, `scan`, `add`, `clone`, `hydrate`, `open`, `sync --hub-file`, `hub gc/login/logout`, `materialize`, `draft snapshot create`, `run-loop`, `worktree new/status/finalize/list/remove/cleanup/unlock`, `env capture/hydrate/bind/rotate`, `run`, `agent run/list/show/pr`, `devices enroll/list/approve/revoke/lost/rename/recipient`, `conflicts list/show/resolve`, `status` (`--watch`), `doctor` (`--remote`), and `db migrate/status/backup/down`.
