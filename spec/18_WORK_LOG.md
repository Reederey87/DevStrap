...
- **`devstrap worktree adopt <path>` registers an externally-created linked git worktree.** Agent harnesses make their own worktrees — Claude Code under `.claude/worktrees/`, the Codex app under `$CODEX_HOME/worktrees`, and anyone at all with a plain `git worktree add`. Until now DevStrap's registry, stale-base gate, and fresh-base provenance were worth something only when `worktree new` had created the worktree, which is the case that matters least. External survey found **no** tool that adopts a worktree it did not create (Claude Code, Codex, Cursor, uzi, Devin, claude-squad, `container-use` all track only their own; `cu checkout` is the nearest analog and is still self-created-only), so this is a differentiated primitive rather than catch-up.
- **The invariant this had to preserve:** adoption is a *registration* plane, never a *base-resolution* plane. It records what a worktree was ACTUALLY based on — `git merge-base HEAD <base_ref>`, **not** the current remote tip. Recording the tip would make every adopted worktree report "fresh" forever, converting the stale-base gate from a guarantee into a lie, and would separately corrupt `agent run`'s `Committed since base:` summary (which diffs `BaseSHA..HEAD`). Verified empirically that merge-base holds under DevStrap's blobless clones: on a genuine partial clone with promisor packs, `merge-base` and `rev-list --count` both return correct results with `GIT_ALLOW_PROTOCOL=none` — offline, no lazy fetch possible. (The first attempt at that check proved nothing and is worth recording: a `file://` clone answers `warning: filtering not recognized by server, ignoring`, so `remote.origin.partialclonefilter` reads `blob:none` while the clone is in fact complete. `uploadpack.allowFilter=true` on the origin side is required for the test to be real.)
- **`git.Runner.WorktreeIdentity`** is now the single definition of "is this a linked worktree", extracted from logic that lived privately inside `WorktreeSandboxWriteDirs`. That function is refactored onto it and translates any resolution error back into its own long-standing `nil, nil` contract, so the sandbox still grants nothing rather than failing — `internal/git/sandbox_writedirs_test.go` passes **unchanged**, which was the merge condition for the extraction.
- **`git.Runner.MergeBase`** classifies git's exit-1/empty-output convention as the distinct sentinel `ErrNoMergeBase` rather than a bare failure, so an orphan-history worktree (a `gh-pages` checkout is the common case) refuses with a remedy naming `--base-ref` instead of an opaque error. **`git.Runner.IsShallow`** gates the case where a `--depth` history can return a plausible-but-wrong merge-base at the shallow boundary — refused unless `--allow-shallow`, which then records a warning rather than staying silent.
- **Detached HEAD is adopted, not refused.** Codex and Devin create detached worktrees by design (Codex deliberately, to sidestep git's one-branch-one-worktree constraint), so refusing them would have rejected most externally-created worktrees in the field and defeated the feature's purpose. `worktrees.branch` is `NOT NULL`; `""` is the representation, and `SafeBranchName("")` is already false so every validating path fails closed.
- **Migration `00032`** adds the `(namespace_id, path) WHERE status='active'` unique index the schema has never had. Without it nothing structurally prevented registering one physical worktree twice — the failure Terraform's own import documentation warns about. The `status` predicate is load-bearing: a removed worktree's path stays legitimately reusable.

Two reap-safety bugs closed, both of which destroy user work:

1. `worktree cleanup --merged` iterated every row with no `created_by` filter and `git branch -D`s what it reaps; `worktree remove` deletes the checkout. Neither is acceptable by default against a worktree DevStrap did not create, so adopted rows are now skipped unless `--include-adopted` / `--prune` is explicit.
2. **The dangerous one, found only by adversarial review of the plan before any code existed.** The merged test is `strings.Contains(mergedOut, wt.Branch)`, and `strings.Contains(x, "")` is **vacuously true in Go**. A detached adopted row carries `branch=""`, so every *clean* one would have read as "merged" the instant `--include-adopted` let it through — deleting exactly the checkouts that flag exists to protect, in exactly the detached case that is the common one. Branchless rows are now never merge-eligible, guarded before the check and pinned by a test named for the regression.
...
Post-review (fable-5, escalated because this touches base-resolution adjacency). One blocker and three real findings:

- **BLOCKER — the PR's central semantic was proven only by prose.** A mutant that discards the merge-base and records `RevParse(baseRef)` (the raw tip — the exact "every adopted worktree reports fresh forever" bug this feature exists to prevent) **passed every adopt test AND `worktree_adopt.txtar`**. Reproduced locally before fixing. The cause is worth recording because it is a general trap: every fixture adopted a worktree sitting at the base ref's current tip, so merge-base and tip were the same commit and the two implementations could not disagree; the txtar advanced the remote only *after* adopt, so both reported `stale (behind 1)`. `TestWorktreeAdoptRecordsAncestorNotBaseTip` now advances the base PAST the worktree before adopting and asserts the recorded base is the ancestor, not the tip — mutation-verified failing under the mutant and passing when restored. spec/14's AD-5 invariant demands a check that fails when the behavior is reverted; until this test the suite did not have one.
- **A real production regression in the sandbox refactor, which the unchanged-and-green test file could not have caught.** Making `WorktreeIdentity`'s main-checkout derivation fatal for a common dir not named `.git` propagated through `WorktreeSandboxWriteDirs`' `nil, nil` contract, so a `--separate-git-dir` clone (reachable via `scan --adopt`) would have had **every** git-storage write denied and the sandboxed agent's own `git commit` would fail with `EPERM`. It fails closed rather than open, which is why it was not a blocker. `MainCheckout` is now `""` instead of an error, `adopt` refuses on that itself, and `sandbox_writedirs_test.go` gains the divergent input its three pre-existing cases never exercised — the point being that its passing proved nothing about this class. `TestWorktreeIdentityBareRepo` was updated to pin the new contract rather than the old one, deliberately.
- **`--project` disambiguates; it does not override.** It was trusted directly, so naming any project registered the worktree under that namespace even when its main checkout was a different repository — corrupting provenance, and `remove --prune` would later run `git worktree remove` from the wrong repo. The named project's own checkout must now match, refused with `exitUsage` otherwise. The mismatch test builds its second project INSIDE the workspace root, because the obvious fixture makes `scan --adopt` refuse and the test would skip — proving nothing.
- **spec/00 factual correction:** it claimed re-running `worktree new` after an adopt "hits the idempotent refresh/no-op path". False — `worktree new` mints a fresh timestamped, randomly-suffixed directory every run and never re-registers an existing one. Only `adopt` has idempotent paths; the index exists to turn a duplicate registration into an error.
- `AD5-01`/`AD5-02` are ticked in spec/14's backlog now that both have shipped.
...
- `gofmt -l cmd internal` clean; `golangci-lint run` — 0 issues.
- `go test -count=1 ./internal/state/ ./internal/git/ ./internal/cli/ ./cmd/...` — all ok (git suite forced non-cached, since the sandbox refactor is exactly the kind of change a cached PASS would hide).
- `cmd/devstrap/testdata/script/worktree_adopt.txtar` confirmed to actually RUN (`-run TestScripts/worktree_adopt -v`), not silently skip.
- `go run ./cmd/spec-drift --base origin/main --head HEAD`.

Follow-ups:

- `AD5-03` (`agent adopt` + `agent finish`) next. Its `--pid` must NOT default to the parent PID: a harness shelling out gives devstrap a transient shell as PPID, and the next sweep would flip a healthy run to `interrupted`. Explicit `--pid` records `processStartTime(<that pid>)`; omitting it means the run is never swept (`RunningAgentRunsWithPID` selects `runner_pid IS NOT NULL`) AND blocks its worktree's cleanup indefinitely — which is why `agent finish` is required scope, not a nicety.
- `doctor` awareness of adopted worktrees whose path has vanished is deliberately deferred to `AD5-06`.
...
- **`spec/14`: new § *AD-5 backlog*, rows `AD5-01`…`AD5-07`.** `AD-5` has been recorded direction since the sixth-pass viability review and restated in `spec/00`, `spec/02`, and `spec/10` — and, alone among this file's DIRECTION entries, never decomposed. AD-1 got rows; AD-8 got an annotated goals list; AD-5 got four restatements of one paragraph. The paragraph did name a build plan, so the gap was never missing intent — it was that the intent was never decomposed into trackable rows with acceptance criteria. The cost is already on the record: the 2026-07-17 wave cited AD-5 as the reason **not** to build a per-harness engine adapter, while nothing decomposed what it licensed building instead. A direction that is only ever actionable as a refusal is not yet a plan. Each row carries an acceptance criterion and an effort size.
...
Research (external, 2026-07-31) that shaped the rows rather than merely decorating them. **Sources are now recorded in `spec/17_REFERENCES.md` § *AD-5 substrate-wave references*** — two rows rest on claims that cannot be checked from this tree (`AD5-02`'s detached-HEAD scope and `AD5-07`'s deferral), and an unauditable rationale is one that rots:
...
- Added `repo.wip.dropped` (`WipDroppedPayload{Path, DeviceID, SHA}`, constructor, apply arm) so a `wip drop` reaches the whole fleet. This closes a gap the Layer B wave shipped knowingly: `wip drop` deleted the remote ref but cleared only the **dropping device's own** mirror row, so peers kept a row for a ref that no longer existed — `doctor` nagged about WIP that was gone and `wip show`/`apply` failed confusingly. Absence-derivation was rejected as the alternative: ref absence is observable only by network probing, so a non-probing device would keep the stale row forever.
- **Verification tier: deliberately NOT in `mustVerifyEvent`, alongside `repo.wip.pushed`.** The destructive framing is illusory — applying a forged `dropped` clears a mirror *pointer*, while deleting the actual ref needs git push credentials to the project's origin, and a forged `pushed` at the same tier already achieves identical denial-of-recovery by overwriting the row's sha so `fetchVerifiedWip` fails closed forever. Splitting the tiers would also create the incoherent state where a device whose pushes apply cannot retract them. Both upgrade together at Phase-2 enrollment; the rationale is pinned in a comment beside `mustVerifyEvent`.
- **Apply is a tombstone, never a row delete** (migration `00031`, schema 30→31: `dropped_at_hlc`, `dropped_sha`). Owner A's `pushed` and dropper C's `dropped` live in different device streams and can arrive in either order, so a row delete followed by a late older `pushed` would re-insert a row for a gone ref. `dropped_at_hlc` takes `MAX(...)` so a redelivered drop is idempotent, `UpsertDeviceWipTx`'s `>=` guard is untouched, and every read applies a liveness predicate instead. Tombstones are permanent: cardinality is devices × projects, and purging would reopen the resurrection window for any `pushed` event still below the hub retention floor. `device_wip` participates in neither snapshot exchange nor hub-compact serialization, so no resurrection path exists through those channels either.
- **The predicate needs three clauses, and the third one is why.** `sha <> dropped_sha` is load-bearing: the lease proves the drop deleted only `dropped_sha`, so an owner who pushed concurrently (HLC 7, sha `def`, not yet having seen `dropped(9, abc)`) still holds live recovery data that an HLC-only predicate would read as absent on **every device including the owner** — and the TTL/GC slice's own-orphan path would then delete it. Equal HLCs from two devices take the same branch (the row's HLC is a bare `int64` with no device tiebreak) and are covered by the same clause.

- **Post-review (coordinator, a real defect the implementation and its tests both missed): the sha escape had to be gated on `observed_at_hlc > 0`, or the tombstone read LIVE.** A `dropped` arriving with no prior `pushed` inserts a tombstone-only row (`observed_at_hlc = 0`, empty `ref`/`sha`) — a routine delivery order, and the *normal* state on a snapshot-bootstrapped device whose pushed event was already compacted away. That row's empty sha differs from `dropped_sha`, so the ungated escape rendered the tombstone as a **phantom pending WIP carrying no ref**: surfaced by `wip status`, `status --all-devices`, and `doctor`, and unusable by `wip show`/`apply` — worse than the resurrection the tombstone exists to prevent, because nothing can act on it. Verified against the raw SQL across all six liveness cases before and after, then pinned by `TestWipTombstoneBlocksOlderPushedResurrection/drop_with_no_prior_push`, which was mutation-checked against the pre-fix predicate (fails; the companion `drop_with_no_prior_push_then_older_push` case is a plain regression guard and passes either way — recorded honestly rather than presented as a second pin).
- **Post-review (coordinator, bookkeeping):** the work-log entry was filed above this file's own *Purpose* and *Entry Format* sections rather than newest-first among the entries.

- **Post-review (dual: Codex + an escalated fable-5 pass, which found the blocker — one BLOCKER, four SHOULD-FIXes, two NITs, all fixed):**
  - **BLOCKER — `dropped_at_hlc` and `dropped_sha` were updated by different rules, so the row could converge on a pair no event ever carried.** The upsert took `MAX()` of the HLC but let the sha be last-write-wins. Reachable with only honest CLI events: A pushes `abc`, B drops it, A re-pushes `def`, C drops that — a fourth device pulling the two drops newest-first ends up holding the newer HLC beside the *older* drop's sha, and the push of the sha that WAS deleted then reads live permanently. The same phantom-pending-WIP symptom this PR exists to remove. Both columns now move together under one `CASE`, with a deterministic sha-order tiebreak so equal-HLC drops converge identically on every device. Pinned by `two_drops_different_shas_newest_first` and its oldest-first twin (mutation-verified against the old rule).
  - **The already-gone drop path was publishing a sha it never leased.** When `DeleteRef`'s lease is refused and `ls-remote` shows the ref absent, the command reports idempotent success — but it proved only *absence*, not which sha was there, and a stale mirror made it assert a sha that was never deleted, resurrecting the row fleet-wide. That path now publishes `SHA: ""`, and an empty `dropped_sha` is the sha-AGNOSTIC tombstone: observed absence outranks any sha comparison. This also retires the predicate's overclaiming "the lease proves" language for that event class. Pinned by `unleased_drop_outranks_a_different_sha` plus a guard that an unleased drop still cannot bury a genuinely newer push.
  - **The txtar's `doctor` assertion could not fail.** `! stdout 'pending WIP'` ran against a WIP created seconds earlier, while doctor warns only past `wipStaleAfter` (48h) — it passed with the whole feature reverted, with the drop never run, and with doctor's predicate broken. Removed, with a comment recording why; the `wip status` assertion beside it is the load-bearing one and does fail on revert. `spec/16`'s matching "no pending/stale WIP" claim trimmed.
  - **The Down migration resurrected every dead row, not just the harmless ones.** The recorded follow-up called out tombstone-only rows downgrading to empty-sha rows; the common case is worse — a normally-dropped row downgrades holding a REAL sha for a ref that is gone, so the fleet's whole drop history re-surfaces in `wip status`/`doctor` and `wip apply` fails on fetch. Schema 30 encodes "dropped" as row absence, so Down now DELETEs dead rows before rebuilding. The round-trip test was itself asserting the wrong behavior (it required the tombstoned row to survive); it now carries one live and one dropped row and asserts preserve-and-delete respectively, mutation-verified.
  - **Recorded a CONSTRAINT on the NEXT slice rather than this one** (`internal/state/store.go`, beside `mustVerifyEvent`): the verification-tier equivalence holds today, and the moment TTL/GC uses mirror state — a tombstone, a row's age, or its absence — as authority to delete a remote ref, the tier stops being a question about pointers and becomes one about credentialed data destruction run by the victim's own device. **The review's framing of that as an *unverified*-event escalation was checked and does not hold:** `verifyEventSignature` gates on `mustVerifyEvent(type) || enrolled`, and `hasEnrolledDevices` is true once ANY device is approved/revoked/lost, so in every workspace with a peer — every workspace a fleet-wide sweep has work to do in — these events are already signature-verified and approval-gated fail-closed (`HUB-03`). This tier governs only the single-device pre-enrollment window, where there is no fleet. Promoting the two WIP types would buy the GC slice nothing, so the constraint recorded is **not** a tier change: mirror state must be a candidate FILTER, never deletion authority, with each delete corroborated by the remote's advertised sha, the ref object's own committer date, and a `--force-with-lease` pin. The genuine residuals are narrower (a revoked device backdating below its own boundary ages only its OWN row; a compromised approved device tombstoning an owner's row is amplification inside the accepted `P7-SEC-05` threat), and the one new security property is that GC upgrades that accepted DoS from denial to destruction — `spec/15` must say so when the sweep ships. The tier note also now says the two events are equivalent in POWER but not OBSERVABILITY — a forged `pushed` fails loudly, a forged `dropped` renders as silent absence on the machine someone went to for recovery.
  - NITs: `safeWipOwnerID` rejected `..` only as a whole string, not as a substring (defense in depth — the real gate is fetch-time `safeRefPath` revalidation, now stated in the comment); `spec/12`'s migration-by-migration enumeration stopped at `00030` while claiming schema 31.

Validated:

- `gofmt -l cmd internal` (empty); `go build ./...`; pinned golangci-lint v2.12.0 (`0 issues`); `go run ./cmd/spec-drift --base origin/main --head HEAD`; `go test -race -count=1 ./...` re-run **natively** by the coordinator, because the delegated worker's sandbox could not bind Unix sockets and reported `bind: operation not permitted` across the daemon/config/platform suites — a sandbox artifact, not a result, and not something to accept on report.
- Mutation checks, each run and each failing the *named* case: predicate `>` → `>=` fails `drop_delivered_twice`; removing `MAX(...)` fails `drop_delivered_twice` (an older repeated drop lowers the tombstone and resurrects the row); removing the sha inequality fails `concurrent_different_sha` and `equal_hlc_different_sha`; removing the `observed_at_hlc > 0` gate fails `drop_with_no_prior_push`; breaking the migration down-rebuild fails the round-trip; removing the drop event emission fails `wip_drop_propagates.txtar` because device A still displays its own dropped row.

Follow-ups:

- Automatic fleet-wide WIP-ref TTL/GC stays unbuilt by design — it is the next slice (`P7-WIP-07`/`P7-WIP-08`), which adds `DeviceWipAll`, the `ls-remote` enumeration primitive, and the expiry policy. `spec/12`'s "TTL/GC remains out of scope" line is left standing until then on purpose.
- The `00031` down-migration preserves every schema-30 column but necessarily drops the tombstone columns, so a tombstone-only row survives a downgrade as a live row with an empty sha. Rollback-only, and noted rather than worked around.
## 2026-07-30 — AGEN-05 part 1: one canonical secret-name predicate

Changed:

- Retired the two duplicated secret detectors — `internal/scan`'s `isSecretName(name, rel)` and `internal/draftbundle`'s `isSecretPath(rel)` — in favor of `ignore.IsSecretName(base)` / `ignore.IsSecretPath(relSlash)` in `internal/ignore`, the canonical `spec/11` compiler package. Both consumers now read one source (`scan.go` walk, `draftbundle.Pack` refusal). This is the scan/draft half of `spec/11`'s "route every consumer through one compiler" direction; **`AGEN-05` itself stays open** — the agent deny-list is the next slice.
- **Exactly behavior-neutral by construction, and the test table is the proof.** `internal/scan`'s `TestIsSecretName` moved into `internal/ignore` as `TestIsSecretPath` **case for case, unchanged**; only the call shape changed. A new `TestIsSecretName` covers the basename-only half, and `TestScanReportsSecretsThroughCanonicalPredicate` pins the scanner's wiring (the detection table no longer lives there, so what remains to test in `internal/scan` is that it reads the canonical predicate at all).
- **The names are a membership test, deliberately NOT entries in `defaultPatterns`** — the `IsOSJunkName` shape, for a stronger reason than the M5D-06 sync argument. Both consumers use this judgement to **act**, not to skip: scan *reports* into `Warnings`/`Secrets` so the user learns a secret is in the tree, and `draftbundle.Pack` *hard-refuses* with a named error, ordered after `matcher.Match` so a user-ignored secret is skipped quietly. Folding `.env` into the ignore table would have turned the first into prune-and-never-report — destroying the signal that is the entire point — and the second into a silent skip. Recorded in the function's doc comment so the next reader does not "simplify" it in.
- **One known gap preserved rather than fixed:** the directory-anchored suffixes (`/.snowflake/config.toml`, `/.aws/credentials`) require a leading `/`, so a `.aws/credentials` at the scan root is still missed. Closing it is a real behavior change; doing it here would have made the behavior-neutrality claim unprovable. Tracked with the agent-denylist slice.
- **Post-review (Codex, one blocker + three should-fixes, all accepted):** `path.Base` is **not** equivalent to the `filepath.Base` it replaced on Windows (`dir\.env` yields no basename), the one input class on which the unification would not have been behavior-preserving — `IsSecretPath` now normalizes with `filepath.ToSlash` first, which is a no-op on Unix and makes the two agree on both platforms. The scan wiring test only exercised `.env`, so swapping the call site for a bare basename check would still have passed; it now includes a `.aws/credentials` fixture, whose basename is unremarkable and which therefore can only be caught by the path-level half (mutation-verified: the basename-only swap now fails it). The migrated table alone was not sufficient neutrality proof, so `TestIsSecretPathMatchesBothLegacyDetectors` compares the new function against **verbatim copies of both retired implementations** across empty paths, trailing slashes, native separators, and the root-level anchored-suffix cases that document the preserved gap. And "byte-identical" was inaccurate — the two clones had different signatures and derived the basename differently; the claim is now "equivalent detection rules for the slash-normalized paths both call sites pass".
- **Post-review (Codex, spec prose):** `spec/11`'s draft-sync paragraph still said the exclusion set MUST be the exact compiler output AND MUST cover secrets, which contradicts itself now that secrets are a separate predicate by design. It now states the effective policy as the compiler output **plus** `ignore.IsSecretPath`.
- `spec/11` corrected in three places: the "Draft sync" current-state paragraph, the stale `defaultPatterns` warning that still described a "hardcoded detector", and the "Agent denylist (unbuilt)" section, which now names precisely what remains divergent (`agentTokenLooksSensitive`, `agentPathLooksSensitive`, `denyParts`, and `internal/platform`'s `sensitiveHomeDirs`/`sensitiveHomeFiles`).

Validated:

- `gofmt -l cmd internal` (empty); `go build ./...`; `go test -race -count=1 ./...`; pinned golangci-lint v2.12.0; `go run ./cmd/spec-drift --base origin/main --head HEAD`.
- **Every new test mutation-checked** by breaking the implementation and confirming the *named* cases fail: dropping the `.env.example`/`.template`/`.schema` exception fails exactly those three rows in `TestIsSecretPath` plus `TestIsSecretName/.env.example`; deleting the `/.aws/credentials` anchored suffix fails exactly `TestIsSecretPath/credentials|work/api/.aws/credentials`; making `IsSecretPath` return false unconditionally fails both consumer wiring pins (`TestScanReportsSecretsThroughCanonicalPredicate`, `TestWalkPrunesGeneratedDirs…`, `TestPackRefusesSecretFiles`, `TestPackRefusesIdRsa`).

Follow-ups:

- `AGEN-05` proper: repoint `internal/cli/agent.go` and `internal/platform/sandbox_profile.go` at the canonical lists, which forces four drift resolutions (`.env.example` currently denied to agents but allowed by scan; `.snowflake` in `denyParts` but absent from `sensitiveHomeDirs`; `*.key` agent-only; the leading-slash gap above).
...
- **Took the FSEvents descriptor measurement** `spec/05` has gated the native-watcher decision on since 2026-07-24, and recorded the verdict as DEFERRED with a standing numeric threshold rather than an open question. Numbers, caveats, and the threshold are in `05_MAC_FIRST_IMPLEMENTATION.md`; the short version: a 37,799-directory tree watches **5,639** directories for **38,095** descriptors (~6.8 per watched directory, confirming kqueue's per-*entry* cost) against the **245,760** this machine's `kern.maxfilesperproc` allows — **15.5%** headroom used. Reconsider at ~20,000 watched directories. The reading is an explicitly-labelled **proxy** (`~/Code` was empty, so the tree measured was the maintainer's working tree; one machine, one macOS version), and the method is recorded so it is re-takeable.
- Re-ticked Milestone 5 in `14_MVP_ROADMAP_AND_BACKLOG.md`, updated `00_START_HERE.md`'s FSEvents claim, and added a **note-only** reconciliation to `docs/audits/README.md` — no row moves and no count changes, since `M5D-*`/`PLAT-*`/`CLI-05`/`ARCH2-*` are spec-space IDs and Pass 7's header count must keep equalling its row count (4).

Three findings from the measurement worth more than the verdict:

- **The cheap fix mattered more than the expensive one.** The same tree under the old hardcoded four-entry prune watches 20,584 directories — extrapolated as a **range** — ~139,000 at the measured source-set ratio, ~157,000 at whole-tree density, i.e. **57–64%** of the limit (the ratio was measured on the pruned source set, while the directories the old rule kept are the file-densest part of a tree, so the single-ratio figure biases low). `M5D-06` cut the descriptor bill ~3.6× for no distribution cost. Had the measurement been taken before that slice, the honest reading would have been much closer to adopting cgo.
- **`launchctl limit maxfiles` reports a soft limit of 256** — a factor of ~4,000 below the shell's `ulimit -n` — and the supervised daemon is exactly a launchd job. The reason it does not bite is **inferred, not observed**: Go raises the soft limit to the hard limit at start (1.19+) and launchd's hard limit is unlimited, but the 245,760 was read from a shell-launched harness, not from a `service install --daemon` process. Recorded because it is invisible until something changes: a non-Go helper, or a Go release that stops doing this, would hit 256 and fail on any real tree.
- **`~/Code` was empty on the measured machine**, so the reading is a proxy taken against `~/Developer` — the maintainer's actual working tree, including this repo, its agent worktrees, and their build output. Stated plainly in `spec/05` rather than presented as a managed-namespace measurement. The conclusion tolerates the proxy because the margin is wide, not because the proxy is exact: the tree would have to grow ~4.5× to reach the reconsider threshold.

- Post-review (a Claude pass over the close itself — **five blockers, all fixed**, and the most useful one was a factual error): the threshold was stated as checkable via `/v1/health`'s `watch.roots`, which is **wrong** — `WatchRoots` returns one path per materialized *project*, so a workspace with a dozen projects reports `roots: 12` while holding thousands of descriptors. The threshold quantity is not exported at all today; that is now said plainly, `spec/13`'s identical pre-existing conflation is corrected, and exporting the count is a tracked follow-up in `spec/14`. Also fixed: `spec/05`'s own heading and two paragraphs still said "DEFERRED pending measurement" / "unquantified" in a file whose `last_reviewed` this commit bumped — the same stale-`spec/05` defect a Codex pass flagged one PR earlier, recurring; the proxy caveat existed once in `spec/05` and in none of the summaries that are read as current status (`spec/00`, `spec/14`, the ledger) — it now travels with every restatement; `245,760` was presented as a property of Go processes rather than of this machine's tunable `kern.maxfilesperproc`; the headroom margin was quoted two ways that disagreed by 45% ("grow ~6×" to EMFILE vs ~4.4× to the threshold); the bolded 57% was falsely precise and one-sidedly biased low, now a 57–64% range with the reason; "two orders of magnitude" for 256-vs-1,048,576 was off by ~2 orders; "one large repository away from `EMFILE`" contradicted the document's own threshold; the launchd conclusion is now marked inferred rather than observed; and the durable record now carries the **method** (machine, macOS version, exact commands, the fact that the tree has no `.devstrapignore`) so the measurement is re-takeable instead of merely trusted. Stale claims the same pass found outside the diff are fixed too: `ARCHITECTURE.md` — the user-facing tier — still described the watcher as gated on a measurement observable via `watch.roots`, false on both halves, and `spec/03` still called the shipped daemon a design target and the native binding "preferred".

Wave close — `M5D-01..07`, PRs #235–#241. The daemon shipped on 2026-07-25 worked, but its own PR bodies recorded a consistent set of follow-ups sharing one theme: **it reported a world more alive than the one it observed.** All of it is now closed: cycle-scoped event publication (so a supervised daemon's event stream is not inert) plus `last_success_at`; `POST /v1/sync`'s mode parameter with `Client.Sync` and `devstrap daemon sync`; the `sun_path` guard and flock-serialized socket takeover (the race #229's own security review recorded as due and never did); a supervised tri-state watch plane that stays alive, re-discovers roots, and retries native; `CLI-05`'s version-header consumption; and `PLAT-01`/`PLAT-04`.

Review notes for the wave, because the pattern is worth keeping:

- **Two merge blockers were tests that could not fail** — both named for the exact regression they were meant to prevent. `TestDaemonStatusReportsProtocolMismatchWithoutFailing` started an ordinary v1 daemon and asserted the mismatch field was *empty*; `TestWatcherDropsWatchForRemovedDirectory` deleted a directory, which both kernels unwatch on their own. A third, of my own writing, passed against a deliberately reintroduced bug because the outcome it awaited still arrived by another route. Since then every fix in this wave was mutation-checked by reverting it and confirming the named test fails, and one proposed test was **rejected** for passing either way.
- **Two blockers were fixes that broke the case they were written for.** The build-skew remedy prescribed `daemon stop && daemon start`, which for the supervised daemon it was designed to diagnose stops convergence rather than restarting it. And the recovery probation added in `M5D-04`'s review cancelled itself against the re-arm cadence, leaving a healthy watcher permanently reported as degraded.
- **The two reviewers found disjoint defects on every PR that had both**, which is the argument for the second pass rather than ceremony. The most consequential finding of the wave — a watcher killed by any transient directory, which would have made `M5D-06`'s entire benefit unobservable *and* this measurement meaningless — came from the pass that ran second.

Validated:

- `gofmt -l cmd internal`; `go test -race -count=1 ./...`; pinned golangci-lint v2.12.0 (`0 issues`); `go run ./cmd/spec-drift --base origin/main --head HEAD`. The measurement itself was taken with a temporary in-package harness (not committed) driving `addRecursiveWatch` against a real tree and counting `WatchList()` plus `lsof` descriptors.

Follow-ups:

- The **job queue** (`ARCH2-04`) stays open by design: what shipped is single-flight coalescing of one job shape, and a real queue is only justified once a caller needs *distinct* jobs. `AGEN-05` (the agent deny-list should read the same `spec/11` compiler — the same unification argument as `PLAT-01`, one subsystem over) is the natural successor, and `spec/05` now names per-project ignore tuning as the cheaper lever to exhaust before FSEvents.
- `doctor` builds most `Detail` strings from raw `err.Error()` (20+ pre-existing call sites); scrubbing them is a file-wide question this wave deliberately did not fold in.
- **CI flake worth watching:** the fuzz-smoke steps (`FuzzParseBytes`, `FuzzCompile`) failed twice in this wave with `context deadline exceeded` after millions of execs and no failing input — a harness deadline during worker shutdown, not a code defect. Both passed on rerun. If it recurs, raise `-fuzztime` headroom rather than treating it as a real hang.
...
- `internal/cli/service.go`: `service install --daemon` bakes `daemon start` into the launchd plist / systemd unit instead of `run-loop`. **No platform-layer rendering changed** — `ServiceSpec.Args` was already a plain argv rendered verbatim into `ProgramArguments`/`ExecStart` — so the whole change is argv construction plus the unit description and log paths (`devstrapd.{out,err}.log`). `serviceRunLoopArgs` and the new `serviceDaemonArgs` are thin wrappers over one shared `serviceConvergenceArgs(head []string, …)`; everything after the subcommand path is identical between the modes, which is the point. The existing `TestServiceRunLoopArgs` passes unchanged, which is the evidence the refactor is behavior-preserving.
- **One label, both modes.** Both install under `DefaultLabel`. Deliberate: one label means one convergence service, so switching modes *replaces* the unit rather than leaving two of them converging against the same state home — a second label would silently double-converge. Installing over a unit in the other mode prints `replacing the existing <mode>-mode unit …`. The label therefore names the convergence service, not the mode, and a code comment says so since the default label is historically `com.devstrap.run-loop`.
- `internal/platform`: `ServiceStatus.Mode`, parsed best-effort back out of the installed unit (`ProgramArguments[1..2]` / the un-quoted `ExecStart` words) on the same degrade-to-empty terms as `ExecPath` — `"run-loop"`, `"daemon"`, or `""` for a unit we did not write. Without it `service status` would keep reporting run-loop framing for a daemon-backed unit, and the mode-change advisory would have nothing to compare against. Surfaced as a `mode:` line and an omitempty `"mode"` JSON field. New goldens `daemon.plist.golden` / `daemon.service.golden` alongside the run-loop pair.
- **The restart-semantics difference is documented, not just coded.** `RestartOnFailure`/30s exists because `run-loop` exits after five consecutive tick failures. `daemon start` never exits on convergence failure — it backs off internally to a 30m cap and keeps serving reads. So under `--daemon` a supervisor restart means a genuine crash, never a failed sync, and convergence health must be read from `/v1/health`; a daemon-backed unit that is "running" is not thereby converging. Stated in the flag's help text, a code comment, and `spec/13`.
- `cmd/devstrap/testdata/script/daemon_lifecycle.txtar` (new) + `cmd/devstrap/script_test.go`: the suite's **first testscript to run a background process**. It covers what the in-process daemon tests structurally cannot — the real process exit code. New `status N <cmd...>` verb asserts an **exact** exit code (testscript's `!` only proves non-zero, and a `!`-only assertion would also pass on a panic; `exitDaemonUnavailable = 3` is the thing worth pinning). New `waitfile` verb makes socket readiness deterministic rather than a sleep. `Setup` exports a short `$SHORTHOME` because `<home>/devstrapd.sock` under testscript's `$WORK` exceeds `sockaddr_un.sun_path`'s 104-byte darwin cap, and the resulting `EINVAL` is deliberately not mapped to `ErrUnavailable` — so the failure would read as a confusing transport error rather than "no daemon". Same precedent as `tempSocketPath` in `internal/daemon/daemon_test.go`; this trap cost three test failures inside this wave.
- `spec/14`: Milestone 5 reconciled against the code. Four tasks flipped to `[x]` (daemon process, HTTP over Unix socket, reconcile job, logs and daemon status) with the PR and mechanism named for each. **Two stay open on purpose and are not bookkeeping debt:** *job queue* (what shipped is single-flight coalescing — one convergence at a time, concurrent triggers join it and share its result — which is not a queue, and `spec/13`'s job types remain design intent) and *native FSEvents* (still gated on the measurement `spec/05` names, which is `/v1/health`'s `watch.roots` count — only takeable now, and not yet taken against a real `~/Code`). The acceptance block named `devstrap daemon install`, a command that never existed and never should: installing a service is `devstrap service`'s job. Replaced with the real loop and marked met.
- `spec/13`: the `service` section documents both supervision modes, the one-label rule, `ServiceStatus.Mode`, and the restart-semantics divergence; also records why the daemon runs foreground under both supervisors (launchd explicitly requires a supervised job not to `fork`+`exit`) and why systemd socket activation is deliberately not used (an activated daemon must not unlink its socket, while `Listen`'s stale-socket takeover and graceful-shutdown unlink both depend on owning it).
- `spec/16`: records the daemon lifecycle e2e under `TEST-02` with the three harness notes above.
- **Trunk-wide truth-up, from review.** The first draft of this PR reconciled only `spec/13`/`14`/`16` and left the *canonical* "genuinely unbuilt" lists asserting the opposite of what had just shipped: `spec/00`'s list still read "daemon, local socket API, FSEvents-specific Mac watcher" as unbuilt (and contradicted its own command inventory 60 lines earlier), `spec/00`'s Current-position line still said "the Phase 1 daemon is gated", `spec/05`/`spec/06` still said "the native daemon (`devstrapd serve`) stays deferred" and that `devstrap service` "targets the shipped `run-loop` today", `ARCHITECTURE.md` still listed the daemon under *What is deliberately not built*, and `README.md` still marked it ⏳. All corrected, each narrowed to what is *actually* still unbuilt (the job model, the endpoints beyond health/version/status/events/sync, native FSEvents) rather than flipped wholesale. The drift gate cannot catch this class — `internal/specdrift` requires only that *some* mapped spec changed — which is exactly why `AGENTS.md` step 2 makes it the author's job.
- `last_reviewed` bumped on `spec/00`, `spec/05`, `spec/06`, `spec/13`, `spec/14`, `spec/16` (all carry substantive status changes).

Changed, from review findings:

- **`service install` now refuses a non-positive `--interval`** (`exitUsage`). The flag meant *opposite* things in the two modes: `runLoopForever` clamps `<=0` up to 5m, while the daemon's `runPeriodic` returns immediately and never converges periodically. So `service install --daemon --interval 0` would have installed an unattended service that silently never converges — with the watcher, itself explicitly "an optimization, never a correctness dependency", as the only remaining trigger. Refusing beats picking a mode-dependent meaning; `daemon start --interval 0` is still available directly for an on-demand daemon. Both modes are pinned by `TestServiceInstallRefusesNonPositiveInterval`.
- **`doctor` no longer frames a daemon unit as "run-loop service".** `checkService` hardcoded the name and never read the new `Mode`, so `devstrap doctor` reported `run-loop service: installed and running` for a `--daemon` install — the identical false framing `ServiceStatus.Mode` was added to fix, in the *other* consumer of the same struct. An unknown/absent mode keeps the historical name, so nothing changes for run-loop installs or unparseable units.
- **`daemon status` now surfaces convergence and watch health.** `spec/13` (as first written in this PR) told operators to read `healthy`/`consecutive_failures`/`last_error` off `/v1/health` — but `runDaemonStatus` fetched `Health` and consumed only `UptimeSeconds`, discarding every one of those fields, and no other command read the endpoint either. The stated workflow was reachable only by curling the socket by hand. Rather than soften the claim, the fields are now reported: `converging: FAILING (<n> consecutive; last error: …)` and `watch: degraded (<reason>)` in human output, plus omitempty JSON fields. `healthy` is emitted only when a `Converger` is wired, so a transport-only daemon does not claim to be failing at convergence it never performs. This matters most under `--daemon` precisely because the supervisor's restart count — the failure signal for a run-loop unit — is meaningless for a daemon that never exits on convergence failure.
- The mode-replacement advisory moved to **after** a successful `Install` and to past tense; announcing it beforehand claimed a replacement that never happened when `Install` failed (`ErrUnsupported` on a headless systemd box). The install confirmation now names the mode, and `serviceInstallResult` gained `mode` so `service install --json` reports what `service status --json` does.
- The systemd keychain-custody refusal still said "the run-loop will exit into a restart loop" — false under `--daemon`, which keeps serving and simply never converges. Both outcomes are now named.
- The testscript verb was renamed `status` → **`exitstatus`**: testscript builtins win the command lookup, so a future go-internal builtin named `status` would have silently shadowed it, and the name also read confusingly next to `exec devstrap status`.

Validated:

- `gofmt -l cmd internal` (clean) · `go build ./...` · `GOOS=linux go build ./...` · `GOOS=windows go build ./...`
- `GOCACHE=/tmp/devstrap-gocache go test -race -count=1 ./internal/cli/... ./internal/platform/... ./cmd/devstrap/...` — green
- `daemon_lifecycle.txtar` run 5× under `-race` plus the full `cmd/devstrap` suite under `-race`: no flakes. The script incidentally pins that `daemon start` exits **0** on SIGTERM and that the socket is reliably gone once that process exits.
- **Live launchd smoke** (not a fake `ServiceManager`): `service install --daemon` under a throwaway label + temp home → the rendered plist carries `[devstrap, daemon, start, --interval, 30m0s, …]` → launchd starts it → the daemon binds its socket and answers `daemon status --json` (`running: true`, real pid) **over the socket** within ~2s → `service status --json` reports `"mode": "daemon"` → re-installing without `--daemon` prints the replacement advisory → `service uninstall` → `daemon events` exits **3**. Machine left clean (no plist, nothing in `launchctl list`, no stray processes).
- `go run ./cmd/spec-drift --base origin/main --head HEAD`
- **Review: three independent passes — fable-5, opus-5, then Codex (gpt-5.6).** Codex was unavailable at first: `codex exec` stalled twice at ~0% CPU with `failed to connect to websocket: HTTP error: 503 Service Unavailable` from the upstream backend, so per the recorded fallback the second pass was a Claude model rather than a retry loop, and the debt was recorded rather than waived. The backend recovered before merge and the Codex pass ran on the final diff, briefed at what the first two had *not* covered (the pointer-bool JSON contract on the new health fields, whether the `interval <= 0` refusal breaks a config-file workflow, and `serviceCheckName` reading `status.Mode` on the path where `mgr.Status` errored and `status` is the zero value): **no defects found.**
- The second pass is why this entry is longer than the first draft. Briefing it to probe what the first likely under-weighted — rather than re-running the same review — produced every finding in the section above: the `--interval 0` divergence, doctor's stale framing, the unread `/v1/health` fields, and the trunk-wide stale "not built" claims.

Follow-ups:

- **A fresh workspace reports a degraded watch plane as its normal state.** The live smoke logged `daemon: watch plane degraded — "no materialized projects to watch yet"` on an empty workspace. That is correct behavior for an empty root set, but it means a new install's *ordinary* state reads as degraded on `/v1/health`, which blunts the silently-degraded-watcher alarm the watcher-wiring slice built. Distinguish "nothing to watch yet" from "the native watcher failed" — track with `PLAT-01`.
- **The SSE stream is inert on a normally-running daemon.** `eventBus.publish` is called from exactly three places, all inside `handleSync` (`internal/daemon/server.go`), so periodic convergence and every watcher-triggered cycle publish nothing; and no `Client.Sync()` or `daemon sync` command exists, so nothing in the product POSTs `/v1/sync` at all. `EventWatchDegraded` is declared in `status.go` and never published. `devstrap daemon events` therefore streams only heartbeats in normal operation. Highest-value item for the next wave.
- **`daemonClientForContext` does not exist** despite being named in the daemon-CLI slice's own description; `runDaemonStatus` and `daemon events` each rebuild the client and the `ErrUnavailable` → `exitDaemonUnavailable` mapping inline. Worth centralizing before a third caller.
- **`doctor` never contacts the daemon.** `daemon status` now reports convergence health, but `doctor` — the command a user runs when something is wrong — still only stats the unit file. A `--daemon` unit that is installed, running, and failing every convergence cycle reads as healthy to `doctor`. Folding the socket's health into `checkService` is the natural next step.
- **`env.Defer` in the testscript `Setup` removes `$SHORTHOME` unconditionally**, including under `go test -testwork` (which preserves `$WORK`). So the one script whose post-mortem state matters most — the daemon's home, DB, logs and socket — is deleted exactly when a developer asked to keep the work dir. `testscript.Env` exposes no `TestWork` accessor, so this needs an upstream-shaped fix rather than a local hack.
- **Pre-existing, found in review, not touched here:** `systemdQuote` escapes `\` only in its quoted branch, so an exec path like `/home/a\b` is emitted unquoted with a raw backslash that systemd would unescape (the new mode parser round-trips it consistently, so mode reporting is unaffected — the *unit* is wrong). And a hand-edited `ExecStart=-/usr/bin/devstrap` (systemd's documented `-` prefix) makes `extractSystemdExecPath` return the prefixed string, producing a bogus `exec_path_missing`.
- **No product-level `sun_path` guard.** A long `--home` is legitimate user configuration, and it fails with a bare `EINVAL`. `Listen`/`NewClient` should check the length and say so.
...
- Post-review (fable-5, no blockers; three should-fixes accepted): (1) `TestPeriodicCyclePublishesConvergeEvents` ran a 50ms interval, so its "no further events" assertion raced tick two — on a starved runner `cancel()` could land after the next cycle published. Now an hour, which loses nothing because `runPeriodic` converges immediately on entry and only then waits. (2) `TestSyncEndpointNoLongerDoublePublishes` did not test its name — a SINGLE POST emitted exactly started+done on the old handler too, so it passed pre-change; the double-publish defect was N CONCURRENT posts, pinned at scheduler level by `TestConcurrentCallersPublishExactlyOneStarted`. Renamed to `TestSyncEndpointPublishesViaScheduler` (its real value is proving `server.events` reaches the scheduler through `New`) with the distinction recorded in a comment. (3) `spec/13`'s `daemon status --json` field list omitted `last_success_at` — drift introduced inside the very file this PR bumped `last_reviewed` on. Also documented that `last_success_at` is the cycle's START time, the conservative bound, so a later reader does not "fix" it to completion time.
- The reviewer independently confirmed the two load-bearing claims: there is no `eventBus.mu` → `scheduler.mu` edge anywhere (so publishing terminal events under `s.mu` cannot deadlock), and `done(N)` strictly precedes `started(N+1)` per subscriber because a starter can only observe `inFlight == nil` after the section that published the terminal event releases the lock.
...
- Post-review (dual review, Codex gpt-5.6 full-stack pass + fable-5 line-by-line; two P1s CONFIRMED and fixed pre-merge): the WIP ref is MUTABLE — its owning device force-pushes it on every `wip push` — so the remote can advance past what this device's synced `device_wip` mirror row records, and the original `show`/`apply`/`drop` all trusted the ref where they should have trusted the mirror. (P1-a, apply/show skew) `wip apply` fetched the ref and ran `git stash apply <ref>` while labeling output with the mirror row's sha: a remote that advanced between the owner's push and this device's last sync would materialize (or display) content no locally-synced, signed record describes, mislabeled as the older sha. Fixed with a shared `fetchVerifiedWip` helper: after `FetchRef`, the ref must `rev-parse` to exactly the mirror row's sha or the command fails closed ("run `devstrap sync` to pull the newer record, then retry"); `apply` and `show` now operate on the VERIFIED sha, never the mutable ref name. (P1-b, destructive drop race) `wip drop` deleted the remote ref unconditionally: a drop driven by a stale mirror would permanently destroy a NEWER recovery snapshot whose `repo.wip.pushed` event had not synced here yet. Fixed by making `DeleteRef` a compare-and-delete — explicit-value `--force-with-lease=<ref>:<sha>` leased to the mirror row's sha (empirically verified against a real remote: correct lease deletes; stale lease rejects with "stale info" and the newer snapshot survives; a lease against an already-absent ref is ALSO rejected) — with the lease rejection disambiguated via a new `Runner.LsRemoteRef` (absent → idempotent already-gone success, preserving the existing txtar's Case-4 contract; moved → refuse with the same sync-then-retry guidance). Pinned by `TestDeleteRefCompareAndDelete` (git-level: stale-leased delete leaves the newer snapshot alive; correctly-leased delete removes; leased delete of an absent ref rejects; unconditional delete of an absent ref stays a non-error) and the new `wip_apply_drop.txtar` Case 6 (end-to-end: B force-pushes a newer WIP without syncing; A's stale-mirror `apply` and `drop` both refuse naming the expected/found shas and leave tree+ref untouched; after B and A sync, the same apply succeeds and materializes exactly the newer content). `spec/07`/`spec/13` updated to document the mirror-row-as-authority doctrine.
...
Changed:
- `internal/cli/wip.go`: `devstrap wip status <project>` reads the local `device_wip` mirror and renders one row per device with pending WIP (device id, ref, short sha, short base sha, "captured N ago" age) via a new pure `wipRowsForProject` helper (mirroring `gitstateRowsForProject`'s no-store/no-I/O discipline) — deliberately returning `nil`, not a forced placeholder row, for zero pending WIP: unlike Layer A's "has anything ever been reported" question, "nothing pending" is the normal, healthy state for most projects most of the time. `devstrap wip show <project> [--device <id>]` resolves a target device (an explicit unknown `--device` is a usage error; no `--device` with exactly one candidate auto-selects it; no `--device` with multiple candidates is a usage error listing them, never a silent guess), fetches its canonically-derived ref (reusing `wipRefFor`/`FetchRef`, never a stored `Ref` string), and displays a stat summary + diff without touching the working tree or index — via `git diff <ref>^ <ref>` rather than a plain `git show <ref>`, since a `git stash create` commit is a merge commit and `git show`'s default combined-diff (`diff --cc`) format renders unusual `@@@`-style hunks; empirically confirmed against a real fixture repo before choosing this.
- `internal/cli/status.go`: `status --all-devices` gains a compact pending-WIP summary line per project alongside the existing Layer A gitstate columns, entirely absent for a project with none (the same zero-is-normal asymmetry).
- `internal/cli/doctor.go`: new `checkPendingWip`, deliberately asymmetric with `checkGitstateFreshness` — a project with no pending WIP, or only recently-captured WIP, renders NO row at all; only a WIP ref older than a new `wipStaleAfter` (48h, shorter than gitstate's 7-day staleness — a stash is expected to be resolved within a session or two, not merely a "has anyone reported in lately" heartbeat) produces a visible warning naming the device and age.

Post-review (coordinator): the delegated write omitted the requested test coverage entirely on first read; while writing it directly, discovered the delegate's own concurrent follow-up pass HAD landed tests (`internal/cli/wip_test.go`, an extended `status_test.go`, and a new `wip_status_show.txtar`) — reconciled by removing the coordinator's now-duplicate `TestWipRowsForProject`/`TestStatusAllDevicesShowsPendingWip` (identical intent to the delegate's `TestWipRowsForProject`/`TestStatusAllDevicesPendingWip`, different names, would have compiled but duplicated coverage) and keeping only `TestCheckPendingWip` (doctor coverage the delegate hadn't written). Independently running the new `wip_status_show.txtar` surfaced two real bugs in the TEST, not the product: (1) an assertion checking that device A's OWN marker string was absent from device A's OWN working tree after `wip show` — impossible by construction, since `git stash create` never touches the working tree in the first place, so A's own uncommitted edit was never going to disappear from A's own file; fixed to a byte-for-byte before/after file comparison, which is what "wip show never touches the tree" actually means. (2) the two-device scenario enrolled device B under an arbitrary chosen label (`devices enroll dev_b ...`) rather than B's own self-generated device id — since every event a device authors is signed with its OWN real id, this mismatch meant A's devices table never held an approved row matching the id B's `repo.wip.pushed` event actually carried, so the pull-apply path silently never applied B's event at all (confirmed by inspecting the raw hub object store and A's local `events`/`device_wip` tables directly: B's event existed encrypted at the hub, but no corresponding row ever reached A's database) — this is a test-construction bug, not a Layer B trust-tier defect (`repo.wip.pushed` is correctly unverified/accept-from-anyone by design; the issue was purely that TestScript's enrollment never established the identity the events actually carry). Fixed by capturing B's real id via `devices list` before enrolling, matching the existing multi-device test idiom used elsewhere in this codebase. `spec/16_TEST_PLAN.md` gains a short "Multi-device testscript convention" note under *Test philosophy* documenting this footgun (two enrollment idioms — a hardcoded label vs. a captured real id — coexist in the suite today; only the latter is safe for joiner→founder event flow) so the next testscript needing bidirectional flow doesn't rediscover it the hard way.
...
- `go run github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2.12.0 run` (0 issues, after a cache clean)
- `go run ./cmd/spec-drift --base origin/main --head HEAD`
- `GOCACHE=/tmp/pr4-final-gocache go test -race ./...` — full suite green, including the corrected `wip_status_show.txtar` (two-device round trip: zero-pending informational messages, single-candidate auto-show with a byte-for-byte working-tree-untouched proof, multi-candidate disambiguation error, unknown-device error) and `TestCheckPendingWip`/`TestWipRowsForProject`/`TestStatusAllDevicesPendingWip`

Follow-ups:
- `devstrap wip apply|drop` (the mutate half).

## 2026-07-17 — fix(git): reject a HEAD symref pointing outside refs/heads/*, close the invariant guard's own gap (P7-WIP-03)

Changed:
- Started as a test-only PR locking in the working-state sync plane's non-negotiable invariant (spec/07, spec/10 § *Independence from the cross-machine sync plane*): the fresh-worktree resolver must never read `refs/devstrap/wip/*`, even when a WIP ref for the same project already sits in the same local repo. An escalated adversarial review (fable-5) of that test-only draft found a genuine, pre-existing production bug the draft's own fixtures did not cover — this entry's final shape is the fix plus a fixture pinning it, not a test-only change.
- **The bug:** `RemoteDefaultBranch` (`internal/git/git.go`) parsed `git ls-remote --symref origin HEAD` with `strings.TrimPrefix(fields[0], "refs/heads/")` — a no-op passthrough when the symref target is NOT under `refs/heads/`. Since `safeBranchName` deliberately tolerates slashes (for real branch names like `feature/foo`), a HEAD symref repointed at ANY other ref shape — including `refs/devstrap/wip/<device>/<path_key>`, even with `refs/heads/main` still genuinely present alongside it — validated as a "branch name" and was returned with a nil error. `symbolicOriginHead` (the `LocalDefaultBranch`/`ResolveDefaultBranch` fallback, reached in production via `git remote set-head origin --auto` writing `refs/remotes/origin/HEAD` from whatever a remote reports) had the identical shape: `git symbolic-ref --short` generically strips any well-known-looking leading segment for ANY target, not only a legitimate `refs/remotes/origin/<branch>` one, so it also returned a WIP-derived string as a "branch". Empirically confirmed via raw git commands before assuming either was real.
- **The fix:** `RemoteDefaultBranch` now requires the exact `refs/heads/` prefix (errors otherwise, naming the rejected non-branch ref). `symbolicOriginHead` now reads the FULL `symbolic-ref` target (not `--short`) and requires the exact `refs/remotes/origin/` prefix before trimming, rather than trusting `--short`'s generic stripping. Both fail closed: a hostile or misconfigured remote can no longer smuggle a "default branch" answer that resolves to anything outside a real branch ref, even when a real default branch exists right next to the poisoned HEAD pointer — the exploit chain traced through to `createFreshWorktreeLocked` would otherwise have fetched the WIP commit into the local object store and, on a repo with a broadened local fetch refspec, silently based an agent worktree on it.
- `internal/git/git_test.go`: `TestRemoteDefaultBranchRejectsNonBranchSymref` and `TestSymbolicOriginHeadRejectsNonRemoteTrackingTarget` pin each fix directly; `TestResolveDefaultBranchAdversarialHeadSymrefWithRealMainAlsoPresent` is the exact end-to-end scenario adversarial review constructed (a real remote with `refs/heads/main` genuinely present AND its HEAD symref separately repointed at a WIP ref) — both `RemoteDefaultBranch` and `ResolveDefaultBranch` (no fallback) now error, while `ResolveDefaultBranch` with an explicit `"main"` fallback still resolves correctly (`DefaultBranchStored`, not `DefaultBranchRemote` — the poisoned HEAD is rejected, not silently trusted), proving the fix doesn't turn a working repo unusable. One pre-existing fake-git-script test (`TestResolveDefaultBranchRepairsAndReadsOriginHead`) needed its hardcoded `symbolic-ref` stub output updated from the old short form (`origin/develop`) to the new full-ref form (`refs/remotes/origin/develop`) to match the changed calling convention.
- `internal/git/git_test.go` also carries the original draft's two fixtures: `TestDefaultBranchResolutionErrorsWhenRemoteOnlyHasWipRef` (a bare remote whose ONLY ref is WIP-shaped) and `TestDefaultBranchResolutionIgnoresLocalWipRefAlongsideDefaultBranch` (a normal repo with `main` AND a locally-mirrored WIP ref both present, plus a `wrapGitWithArgLog` helper asserting the resolution+fetch subprocess calls never mention `refs/devstrap/wip/*`).
- `cmd/devstrap/testdata/script/worktree_never_bases_off_wip_ref.txtar` (new): the adversarial e2e precondition end to end — device A pushes WIP content carrying a unique marker string; device B syncs, then `wip fetch`s A's ref into its OWN local repo; device B then creates a fresh worktree for that same project via `worktree new --fresh-upstream`. Asserts the new worktree's checked-out content omits the marker, the recorded base sha is NOT the WIP push's sha, and the WIP ref itself survives untouched afterward.
- `spec/10_AGENT_WORKSPACES_AND_POLICIES.md`: the independence-from-sync-plane paragraph now cites this regression coverage by name.

Post-review (fable-5 escalated adversarial pass, per this project's established pattern of escalating review on sync/trust-plane-adjacent findings): found `RemoteDefaultBranch`/`symbolicOriginHead`'s unguarded prefix-trim (above) — the one real gap in the test-only draft, empirically reproduced before being reported. Also independently verified, with no gap found: (1) exactly two callers ever create a fresh agent worktree (`worktree new`, `agent run`), both funneling through the single `createFreshWorktreeLocked`/`resolveWorktreeDefaultBranch` path this suite covers; (2) multiple WIP refs present locally can't matter — every git call in the resolution+fetch+worktree-add path is scoped to a single named ref or the `HEAD` symref, never an enumeration that could pick up an arbitrary ref by accident.
...
- `go run github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2.12.0 run` (0 issues, after a cache clean)
- `go run ./cmd/spec-drift --base origin/main --head HEAD`
- `GOCACHE=/tmp/pr3-fix-gocache go test -race ./...` — full suite green, including all new/updated tests and the new testscript

Follow-ups:
- `devstrap wip status|show` + `status --all-devices`/`doctor` surfacing (the read/inspect half).
- `devstrap wip apply|drop` (the mutate half) — `wip apply` must itself never be reachable from agent worktree-base resolution either; this PR's coverage is scoped to the RESOLVER side, not a future `apply` command's own safety.
...
Post-review, second pass (coordinator, found while independently verifying the `PushRef` fix's blast radius before propagating it through the rest of the wave): `FetchRef` (this PR's own addition) has the exact mirror-image bug on the pull side. Since a WIP ref is force-pushed on every `wip push` (each fresh `git stash create` commit is a sibling of the previous one, never its descendant), a device that already fetched a ref once and then fetches again after the owning device pushed a newer snapshot hits the identical non-fast-forward rejection — empirically reproduced with two sibling `commit-tree` commits pushed to the same ref, then fetched twice. Fixed with the same `+` force-refspec prefix on the fetch source. Neither `internal/git/git_test.go` nor any testscript exercised a second fetch of an already-fetched ref before this — added `TestFetchRef` (basic contract, previously untested at the unit level entirely) and `TestFetchRefForcesNonFastForwardLocalUpdate` (the regression pin, mirroring `TestPushRefForcesNonFastForwardUpdate`'s sibling-commit construction).
...
Post-review (CodeRabbit, PR #220 thread; fixed pre-merge): an EMPTY `sha` argument would turn `PushRef`'s refspec into `+:<ref>` — git's DELETE syntax — silently destroying the remote WIP state if a caller ever ignored `StashCreate`'s `ok==false` return. `PushRef` now validates the sha as a full-length hex object id (`isHexObjectID`, 40/64 lowercase hex — also rejecting abbreviations, revision expressions like `HEAD`, and option-shaped strings), pinned by `TestPushRefRejectsNonObjectIDSHA`.
...
- Post-review (Codex + opus-4.8, both converged on the same bug plus two smaller follow-ups): (1, real bug) `checkGitstateFreshness` silently `return nil`'d on a `store.ListProjects` error, making the entire check vanish from `doctor` output instead of warning — directly contradicting its own "never silent all-clear" doc comment. Now mirrors `checkFailedMaterializations`'s handling of the identical error: returns one visible `{Name: "gitstate", Status: checkWarn, Detail: err.Error()}` row. (2, same root cause, opposite failure mode) `renderAllDevicesStatus` aborted the whole render (`return err`) on the FIRST project whose `DeviceGitstateForProject` call failed, losing visibility into every other already-successfully-read project too. The per-project row-building logic is now extracted into a pure `gitstateRowsForProject(rows, err, now) []deviceGitstateRow` helper called from a loop that always appends and never returns early, so a failed project renders a visible `error: ...` row (matching the `never synced` row's shape) and the loop continues. (3, comment correctness) the removed local `gitstateHLCLogicalBits = 16` constant's doc comment claimed it avoided "an import cycle" — there is none (`internal/cli` already imports `internal/state` one-directionally). Replaced the duplicate bit-math with two new exported helpers on `internal/state`: `HLCPhysicalTime(hlc int64) time.Time` (wraps the existing unexported `unpackHLC`) and its inverse `HLCFromPhysicalTime(t time.Time) int64` (wraps `packHLC`, used by tests to construct fixture HLC values) — removing the third independent copy of the 16-bit width (`internal/state`/`internal/sync` still each keep their own for unrelated HLC-arithmetic reasons; this was purely a read-back helper with no reason to re-derive the width).
...
- New tests: `internal/cli/status_test.go` — `TestStatusAllDevices` (end-to-end through the real binary: a project with a `device_gitstate` row renders `last seen`/the device id in both human and `--json` output; a project with zero rows renders an explicit `never synced` row in both — the never-silent-all-clear assertion is `never.Devices[0].Observed != "never synced"`), `TestGitstateRowsForProject` (post-review: the error branch produces one visible `error: ...` row, never an empty/aborted result); `internal/cli/doctor_test.go` — `TestCheckGitstateFreshness` (zero-row project produces `checkWarn` containing "no device has reported"; a stale 8-day-old observation produces `checkWarn` naming the device; a fresh 1-hour-old observation produces `checkOK`), `TestCheckGitstateFreshnessSurfacesListProjectsError` (post-review: an un-migrated store forces a real `ListProjects` error, asserting the check still returns exactly one visible warning row rather than silently vanishing).
...
Changed:
- `internal/cli/forge_pr_test.go` (new, test-only — `internal/cli/forge.go` and `internal/cli/doctor.go` are untouched): closes FORGE-05, the audit finding that the actual `gh`/`glab`/`tea` invocation paths had zero dedicated hermetic coverage (only the SSH-alias-resolution half of forge routing was tested, `P6-QUAL-04`). Added `stubForgeCLI(t, binName, exitCode, stdout, stderrMsg)`, generalizing the existing `stubSSH`/`stubOp` PATH-shim pattern to gh/glab/tea: the stub records its full invoked argv to `invoked_args.txt` so tests assert the exact command shape, then emits the given stdout/stderr and exit code.
...
- `go run github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2.12.0 run ./...` — 0 issues (after clearing a stale cache holding gosec/staticcheck ghosts from an unrelated, already-deleted sibling worktree).
...
### 2026-07-15 — review fixup (P7-PROD-01 slice 2): CodeRabbit findings

Auto-merge on PR #202 sat blocked for hours despite green CI because the orchestrator had not been checking CodeRabbit's review threads (a process gap; `AGENTS.md` requires every thread, including CodeRabbit's, resolved before auto-merge unblocks). Two real Major findings, both fixed:

- `up`'s `--hub` preflight went through `hubConfigured`, which returns success immediately whenever a `hub-file` is ALREADY configured (correct for that helper's own purpose — resolving which backend `sync` will use) — but that meant an already-set `hub-file` silently bypassed validation of the explicit `--hub` value entirely, so a malformed `--hub` could found the workspace and get persisted before anything ever parsed it. Extracted the URI-shape switch into a standalone `validateHubURI` that `up` now calls directly on its own candidate, independent of any unrelated `hub-file` setting.
- `runInit` resolved a positional `[root]` argument only into its own local variable, never writing it back into the shared viper config — so `devstrap up /custom/root --hub …` could initialize one root via `runInit` but then scan/sync a stale DEFAULT root, since `up`'s later steps (`runLoopScanAdopt`, `rewriteConfigHub`, `runSyncCycle`) each call `opts.paths()` fresh rather than reusing `runInit`'s local value. Fixed with one `opts.v.Set("root", cleanRoot)` right after `runInit` resolves the final absolute root — every later same-process `opts.paths()` call now sees it. New regression test (`TestUpPositionalRootPropagatesToScanAndSync`) verified to actually fail without the fix (adopted 0 projects) before confirming it passes with the fix.

Five Minor findings also fixed: `docs/quickstart.md`'s manual-fallback wording now covers a carried local `file:`/`folder:` hub, not just "no remote hub"; `TestPairApprovesPastedJoinerCodeAndSyncs` now asserts the joiner's actual fingerprint is displayed (previously discarded) and matches the published grant to the joiner's specific recipient (previously accepted any `device.key.granted` event); `TestUpRefusesExistingJoiner` now snapshots the full config before/after instead of checking one substring's absence; `TestUpRejectsEmptyFileHub` now also checks `config.yaml` (not just `state.db`) and fails on any unexpected `Stat` error.
