...
- Filed #177 (follow-up, not a blocker): the same job executes `./native/devstrap version` before running `cosign verify-blob`/`slsa-verifier verify-artifact`, so an as-yet-signature-unverified binary runs before the later steps confirm it; worth reordering verification before execution in a future PR.
...
- Dual review: Grok-4.5 (mechanical fix + validation), Codex adversarial review (confirmed no narrower permission fix exists via GitHub's documented list-endpoint draft-visibility restriction; validated the `stable-publish` precedent; flagged the pre-existing execute-before-verify ordering as a non-blocking follow-up, filed as #177).

Follow-ups:
- #177 — reorder `stable-smoke` to verify cosign/SLSA before executing the downloaded binary.
