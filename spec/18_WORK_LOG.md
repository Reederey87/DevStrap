...
- `.golangci.yml` exclusions: added `contextcheck` to the existing `_test\.go` rule (test helpers legitimately create fresh root contexts), and added one narrow, heavily-commented `internal/cli/root.go` exclude-rule. The latter is documented explicitly as a **known cobra/linter false positive, not a blanket suppression**: `Execute(ctx)` calls `root.SetContext(ctx)` and every `RunE` sources its context from `cmd.Context()` (verified end-to-end in `init.go` — every call threads `cmd.Context()`, no `context.Background()` misuse), but `contextcheck` cannot trace inheritance through cobra's `SetContext`/`cmd.Context()` indirection and so flags `NewRootCommand` (built without a `ctx` param) as failing to pass the context.
- `spec/13_CLI_DAEMON_API.md`: documented the context threading in the `open` (editor) and `agent pr` (forge/`ssh -G`) paragraphs.
- `docs/audits/README.md`: `P4-QUAL-07` moved to *Recently shipped* (full close — all three linters now enabled); the Pass-4 open count decremented; the ledger row notes the documented-exclusion approach so a future auditor does not reopen it as a suppression.
