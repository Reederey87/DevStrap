...
**Accepted residual — the owner's own clock (`P9-WIP-03`, open).** That last
...
### Threat: hub silently withholds/truncates a device's newest events (`P4-SYNC-05`, mitigated)

Attacker = the untrusted hub, omitting the TAIL of an origin device's stream (serving events 1..N but withholding N+1..M). Unlike the byzantine forge-at-slot case above, plain tail-withholding leaves the retained prefix internally consistent — the `prev_event_hash` pointer chain never points forward, so nothing in the events themselves reveals the missing tail, and a pulling device could converge on a stale view believing it was complete. **Mitigated (`P4-SYNC-05`, shipped):** each device folds its own stream into a running commitment and publishes it as a SIGNED per-device head (the v2 ack's `folded_hash` at `pushed_through_seq`, Ed25519-signed under `devstrap:ack:v1`). On the pull path, `VerifyPeerHeads` compares each approved peer's signed head against the contiguous prefix this device folded: a head that commits to a seq beyond what we hold — persisting past a one-cycle in-flight-race grace — raises a `withheld_tail` `event_omission` conflict, and a fold that disagrees at a seq we DO hold raises a `fork`. The promise is monotone per peer (persisted in `device_heads`), so a hub cannot retract it by serving a stale ack. The alarm is LOUD (a durable `event_omission` conflict that blocks `hub gc`) and integrity-preserving (it flags incompleteness; it never applies unauthenticated content). It resolves when the peer's fold later catches up (a backfilled tail no longer blocks gc), and `hub compact` — the documented cure for a *permanent* gap — is deliberately NOT blocked by an open omission alarm (blocking the cure on the gap's own alarm would make `P5-SYNC-01` recovery unreachable). A gap this device itself declined (an unreadable cross-epoch key-grant carrier, a consumed skew/hash-chain/verification quarantine) is classified as a LOCAL decline, not a withhold, so an honest hub raises no alarm during a routine key-grant grace window.

**Scope of the guarantee — what this detects, precisely.** `P4-SYNC-05` detects an **inconsistent** view: a hub caught contradicting itself, by serving a signed promise (an ack committing to seq M) that is fresher than the event prefix it is willing to back (only 1..N, N<M), or by serving a fold that forks from ours at a seq we hold. It does NOT detect omission in general. **Residuals (accepted):**

- **Consistent stale / frozen view (undetectable here).** A hostile hub can serve a victim a matching, internally-consistent STALE pair *forever*: an old ack@k (valid signature, correct fold for k) together with only events 1..k, silently withholding BOTH the newer ack AND the newer events in lockstep. Nothing contradicts anything — the promise exactly matches the held prefix — so zero alarms fire, indefinitely, per-victim. This is the classic split-view problem that Certificate Transparency itself closes only via gossip / third-party auditors; DevStrap has **no such gossip or external auditor** (out of scope by design), so a hub that freezes a device's whole view in lockstep is not caught by this mechanism.
- **One-promise-lag bounded staleness (undetectable here).** Because reaching a previously-promised head renews the grace, a hub that relays acks but paces/delays the underlying events can keep a victim permanently exactly-one-promise stale: each cycle the victim reaches the OLD promise (grace renews, `caughtUpToOldPromise`) while trailing the newest one by a legitimate-looking single cycle. No alarm ever fires. This is a real, bounded, hub-paced staleness channel, not merely "a peer that never acks."
- **Best-effort detection.** A peer that never publishes a v2 head (un-upgraded, or a device that never completes a clean cycle so never writes an ack), or a stream this device cannot seed a fold for (compacted below the floor without a `folded_hash` anchor seed), is skipped rather than falsely alarmed; and the one-cycle grace means a withhold lasting a single pull is indistinguishable from an in-flight race.

A hub that withholds objects can always deny availability; a zero-knowledge transport was never an integrity guarantee against omission. What `P4-SYNC-05` removes is narrower than "the silent-truncation class in general": it makes a hub that presents an INCONSISTENT view (a fresher promise than it backs with events, or a forked fold) detectable and loud, where previously tail-truncation left no client-visible trace at all. It does NOT close the consistent-stale-view or one-promise-lag channels above.
...
### Threat: a single compromised approved device can revoke the fleet (`P7-SEC-05`, accepted tradeoff)

Attacker = a single compromised-but-approved device with valid signing keys (already in the adversaries list: "malicious approved device with valid keys"). Via TRUST-01's synced trust plane, that device can emit `device.revoked`/`device.lost` for every OTHER approved device; each receiver applies the sticky/monotonic trust flip (local device exempt), and — since `P7-SYNC-04` — every device that merely LEARNS of a revoke also owes a WCK rotation (`wck_rotation_pending`, epoch+1). There is no quorum, rate-limit, or confirmation gate: one approved signer can lock out all legitimate peers and churn the workspace content-key epoch fleet-wide. **Accepted tradeoff** of the zero-knowledge, no-central-authority model: a server-side quorum check would require a trusted third party that can override or gate self-authorized approved devices, which contradicts the corpus-wide design that the hub is untrusted and only client-side crypto matters. The intentional asymmetry bounds the blast radius to denial, never attacker onboarding: `device.approved` is deliberately never propagated (approval stays the local `P4-SEC-04` fingerprint ceremony), so a compromised approved device can REVOKE fleet-wide but cannot ADD/APPROVE attacker devices fleet-wide. **Partial mitigations (after-the-fact auditability only — none of these PREVENT the DoS):** `doctor` reports a "device trust" check (`"%d approved, %d revoked/lost, %d pending"`), so a mass-revocation shows as an unexpected jump in revoked/lost; `devices list` surfaces each device's `trust_state`; `secret_bindings.needs_rotation` is flagged once on apply and `doctor` warns when a revoke-path WCK rotation is still owed ("a device revoke could not rotate the workspace key…"); a still-pushing revoked device's traffic quarantines as `event_verification_failure` conflicts visible in `conflicts list` (bounded aggregation). **Recovery:** local re-approval (`P4-SEC-04`) on each falsely-revoked device; mutual/cross-window divergence recovers via the two-step local re-approval already documented under the `P6-SYNC-01` residual (re-approving one side replays its counter-revoke).
...
### Threat: a local process drives the daemon's control socket (M5 transport, mitigated + documented residual)

The daemon (`internal/daemon`, shipped 2026-07-24) introduces DevStrap's **first listening socket** — `~/.devstrap/devstrapd.sock`, HTTP+JSON over a Unix domain socket. Anything that can reach it can ask the daemon to act on the workspace, so it is a new local attack surface and worth stating plainly.

Mitigation, layered in the order an attacker meets them:

- **Filesystem.** The socket's parent directory is forced to `0700` and the socket to `0600`. The directory is the load-bearing control: a peer that cannot traverse it never reaches the socket at all. (The socket's own mode is applied just after `net.Listen`, so it briefly carries umask-derived permissions — another reason not to rely on it alone.)
- **Peer credentials.** Every connection's uid is read at accept time (`platform.PeerCred` — `LOCAL_PEERCRED` on darwin, `SO_PEERCRED` on linux) and must equal the daemon's own uid. **Root is not exempt**: root can open a `0600` socket it does not own, which is exactly the case filesystem permissions cannot cover, so this layer is what stops it. The check is **fail-closed** — any error, including `ErrUnsupported` where credentials cannot be read at all, refuses the connection rather than falling back to permissions alone. (Contrast `ProcessAlive`, which is deliberately fail-*safe*; the two seams answer different questions.)
- **Request shape.** Requests carrying `Origin`/`Referer` are refused outright, so a page the user happens to have open cannot drive the API even if it reached the socket; the `Host` header is pinned; bodies are bounded; `ReadHeaderTimeout` is set.
- **Blast radius today.** The shipped surface is read-only (`/v1/health`, `/v1/version`) and carries no secret material. Mutating endpoints arrive in later slices of the wave and inherit these controls.

**Accepted residual, stated explicitly:** any process running as **the same uid** can drive the daemon. This is not a new grant — such a process can already read `state.db`, the file-backed key store, and any hydrated `.env`, and can simply run `devstrap` itself. The daemon does not widen the trust boundary; it makes the existing one reachable over a socket. DevStrap does not attempt to defend a user's account against code already running as that user.

**A sandboxed agent is inside this residual, not outside it.** It would be convenient to claim the OS sandbox (`P4-GIT-03`) contains an agent here, and that claim would be wrong under the default policy. Write-confinement of `~/.devstrap` does not stop a `connect()` to a socket: on macOS a unix-socket connect is a `network-outbound` operation under Seatbelt, not `file-write*`, and the generated profile only emits `(deny network*)` for the `readonly`/`cautious` policies; on Linux the default bwrap shape read-only-binds `/`, which likewise does not block connecting to a socket (only `~/.devstrap/keys` is masked, and read-confinement is auto-enabled for `readonly` alone). The codebase already concedes the class elsewhere — the partial-network-deny warning states that unix-domain sockets stay open. So an agent running under the default `guarded` policy **can** reach the socket and **will** pass the peercred check, because it genuinely is the same uid and that check cannot distinguish it from the user.

Today the blast radius is nil — the surface is `GET /v1/health` and `GET /v1/version`, which carry no secrets and change nothing. That stops being true when mutating endpoints land. Before then, either the sandbox profiles must deny the socket path explicitly, or a mutating endpoint must require something an agent does not have (the peercred uid is not sufficient). Recorded here rather than in a later slice's notes because it is a property of this boundary, and a later reader would otherwise reasonably assume the sandbox covers it.
...
- **SECU-02**: `SSH_AUTH_SOCK` excluded from agent subprocess environment via `AgentAllowlist`; HOME repointed to the worktree so RELATIVE dotfile lookups (`~/.ssh`, `~/.aws`, `~/.snowflake`, `~/.config/gh`, `~/.config/gcloud`, `~/.azure`, `~/.git-credentials`) miss. This is environment isolation only — it does NOT stop an agent reading the REAL home by absolute path. Absolute-path credential reads are denied by the OS sandbox's canonical credential lists (`ignore.CredentialHomeDirs`/`CredentialHomeFiles`, `AGEN-05`) on all three backends — macOS Seatbelt and Linux bubblewrap mask them, and the Linux Landlock fallback now denies them by default via leaf-hierarchy read grants that omit the anchors (`P7-SEC-03`, `EACCES`), no longer requiring `--read-confine`. This includes `~/.snowflake`; guarded `snow` CLI use therefore requires an explicit `--policy yolo-local` or `--sandbox off` escape. `--sandbox auto` can still degrade to advisory behavior on hosts with no sandbox adapter. Use `--sandbox require` to refuse an unenforced run.
