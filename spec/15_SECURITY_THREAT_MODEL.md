...
`P4-SEC-04` part 2 is now shipped: `devices pairing-code` emits a one-paste `devstrap-pair1:` blob carrying workspace id, device id, display name, OS, arch, age recipient, and signing public key; `init --join --code` adopts the workspace id and pins (or pending-enrolls) the founder; `devices enroll --code "$CODE" --approve --fingerprint "$FP"` performs founder-side enrollment in one command. The blob is **deliberately unauthenticated** and carries no fingerprint. Tampering with any carried key changes the derived fingerprint, so the out-of-band confirmation catches it; tampering with workspace/device/name/OS/arch is rejected by validators or surfaces as the wrong displayed target. This closes the local pairing-channel residual from parts 1+2; authenticated snapshot exchange and remote trust propagation remain separate future work.
