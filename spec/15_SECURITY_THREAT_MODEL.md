...
- **[SHIPPED — PR-3b]** a WCK keyring keyed by `(epoch, kid)` where `kid = hex(sha256(wck))` (full
  ...
  grant-preferring push-key selection (`P6-SEC-01`/`P6-SEC-02`); the remaining break item is
  moving the kid inside the `enc.v2` AAD;
