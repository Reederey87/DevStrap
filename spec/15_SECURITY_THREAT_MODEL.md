...
- OS sandboxing is required before public release — macOS Seatbelt, Linux bubblewrap, Linux Landlock fallback, the Linux seccomp syscall denylist, and unsigned local `sandbox.violation` telemetry shipped 2026-07-05 (`P4-GIT-03`, default-on via `--sandbox auto`); the remaining slice is tighter read confinement.
