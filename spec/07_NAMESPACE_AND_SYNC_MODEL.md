...
repo.wip.pushed               # pushed WIP pointer (working-state validation plane — Layer B)
repo.wip.dropped              # fleet-wide tombstone for a lease-deleted WIP pointer
conflict.created
conflict.resolved
```
...
- **Layer A — validation (Phase 0, SHIPPED `P7-GITSTATE-01`):** each device emits `repo.gitstate.observed` (branch, HEAD sha, upstream sha, dirty/untracked/unmerged/ahead/behind/stash counts), captured with `git --no-optional-locks status --porcelain=v2 --branch` so capture never writes `.git/index` (`internal/git.Runner.CaptureGitstate`; upstream SHA and stash count are two cheap follow-up calls). Apply is mirror-only into a sidecar `device_gitstate` table (migration `00029`; opaque `device_id`, **no FK** to `devices` since remote devices are not enrolled until Phase 2 — mirrors `device_heads`; also no FK to `namespace_entries`, since this mirror does not need the observed project to exist locally to apply). Shipped: the capture routine, the `repo.gitstate.observed` constructor/apply handler, the `device_gitstate` storage, the `status --all-devices`/`doctor` read side (`status --all-devices` renders every device's last-observed working-state per project, and `doctor` grades staleness — both always render an explicit row per project, "never synced" when zero devices have reported or "last seen N ago" otherwise, never a silent all-clear; see `13_CLI_DAEMON_API.md`), and (as of this PR) the runtime wiring: `devstrap sync`'s `captureAndRecordGitstates` step captures every already-materialized `git_repo` project once per cycle, mirrors the observation into this device's own `device_gitstate` row in the same transaction as the local event insert (a remote peer's `ApplyEvents` dedups an event ID already present locally and never re-applies it, so without this explicit local mirror write a device's own observation would never appear in its own `status --all-devices`), and skips emitting when the capture is unchanged since the device's last-recorded mirror row (`gitstateUnchangedSinceLastCapture`) so an idle sync stays a zero-push no-op (`SYNC-04`). The capture step runs before eager materialization in the same cycle, so a project newly materialized THIS cycle is captured for the first time on the NEXT sync, not the same one.
- **Layer B — WIP recovery (Phase 1, SHIPPED `P7-WIP-01`–`P7-WIP-06`):** `git stash create` (no worktree/index mutation, `internal/git.Runner.StashCreate`) → `git push origin <sha>:refs/devstrap/wip/<device_id>/<path_key>` over a raw refspec push (`internal/git.Runner.PushRef`, validated by a namespace-scoped `safeRefPath` distinct from the generic `safeBranchName`) → emit `repo.wip.pushed` (`internal/sync.NewWipPushedEvent`). Forge-agnostic. Apply is mirror-only into a sidecar `device_wip` table (migration `00030`; same no-FK rationale as `device_gitstate`/00029), and `repo.wip.pushed` sits at the same attributed-but-unverified trust tier as `repo.gitstate.observed` (neither is in `mustVerifyEvent` until Phase-2 device enrollment). The full CLI is shipped (see `13_CLI_DAEMON_API.md` § *WIP commands* for the full contract): `devstrap wip push <project>` is the write half; `devstrap wip fetch <project> [--device <id>]` mirrors a peer's ref locally, always deriving the ref canonically from device id + path_key (never trusting a stored/peer-supplied `Ref` string) and, without `--device`, discovering candidates from the synced `device_wip` mirror rather than probing the network; `devstrap wip status <project>`/`devstrap wip show <project> [--device <id>]` are the read/inspect half (an empty mirror is the normal, healthy state — a plain informational line, never a warning, unlike Layer A's forced "never synced" row); `devstrap wip apply <project> [--device <id>]`/`devstrap wip drop <project> [--device <id>]` are the mutating half. Because the WIP ref is MUTABLE (its owning device force-pushes it on every `wip push`), the synced `device_wip` mirror row is the authoritative record of what a peer's ref is supposed to contain, and both `show` and `apply` verify it: after fetching, the ref must resolve to exactly the mirror row's sha, else the command fails closed ("run `devstrap sync` to pull the newer record") rather than showing or materializing content no locally-synced record describes. `apply` then runs `git stash apply <verified-sha>` with no pre-emptive dirty-tree gate (git's own per-file conflict detection already refuses safely, either aborting cleanly or leaving standard conflict markers, distinguished via `git.Runner.DirtyState`), materializing into the worktree **only on this explicit command, never as a branch or a fresh-worktree base** — the non-negotiable invariant below. `drop` is a COMPARE-AND-DELETE for the same reason: `git.Runner.DeleteRef` leases the delete to the mirror row's sha (explicit-value `--force-with-lease`), so a stale drop can never destroy a newer recovery snapshot whose event has not synced here yet; a lease rejection is disambiguated via `git.Runner.LsRemoteRef` (already-gone → idempotent success; moved → refuse until `devstrap sync`). `drop` emits `repo.wip.dropped`, permanently tombstoning the owner's mirror pointer fleet-wide while its leased old SHA keeps a racing different-SHA push live across cross-stream ordering; automatic TTL/GC **shipped** (`P7-WIP-07`/`P7-WIP-08`): `devstrap wip gc` expires aged refs under a corroborated, leased delete, and the sweep rides the single sync/run-loop/daemon convergence path, interval-gated and failure-isolated. (Corrected 2026-07-31, `P9-WIP-04`: this sentence said "remains unbuilt" while the same file's own Layer-B expiry text described the shipped behavior.)
...
**Version-skew policy (`P7-PROD-03`).** Retention manifests are read across an inclusive **N-1 window** — `[minReadableRetentionManifestVersion, snapshotVersion]` in `internal/sync/snapshot.go` — rather than the two hardcoded versions the v1/v2 bump originally shipped with: `retentionManifestVersionOK` is a genuine range comparison, so a future version bump only moves the two named constants, never re-derives the check. This exists because a mixed-version fleet is a normal, unavoidable state (a `brew upgrade` landing on some machines before others) and the alternative — `hub compact`/`doctor --remote` erroring outright on a manifest one version behind — would wedge convergence instead of degrading it gracefully. The manifest additionally carries an optional, **signed** `min_reader_version` stamp: a future producer that ships a genuinely breaking manifest change can declare "readers below N must upgrade" explicitly rather than relying solely on readers pre-emptively widening their own hardcoded range; it is signed (not advisory/unsigned metadata) because an unsigned "you must upgrade" field would let an attacker who can write hub objects — but not forge signatures — manufacture a denial-of-service by forcing every reader to self-reject a legitimately-readable manifest. `ParseRetentionManifest`/`VerifyRetentionManifest` fail closed if this binary's own `snapshotVersion` is below a manifest's declared `min_reader_version`. `doctor --remote` surfaces skew as a WARNING, never a failure: it reads the manifest's version stamp directly (bypassing the normal fail-closed parse, so it can explain a wedge instead of hitting it) and flags either direction — the hub's manifest is behind what this binary produces, or this binary is behind a `min_reader_version` floor the hub has declared.

This policy is deliberately scoped to the retention MANIFEST only. The **snapshot document/envelope check stays exact-equality fail-closed** and is explicitly NOT part of this window (see the `Snapshot object` paragraph above): a v1 snapshot silently lacks the `P7-SYNC-01` device-trust projection, so accepting an old snapshot DOCUMENT would be a live security regression (a revoked device could stay approved on a recovering device), not just a compatibility inconvenience. Manifests are trust-neutral (floors only), which is exactly why their read window can widen safely while the document's cannot.

If a retention-manifest change is ever genuinely breaking — not just an additive, ignorable version stamp — this repo's existing precedent is to ship an in-place migrator rather than force a re-found/rebuild: `devstrap hub migrate-events` (`internal/cli/hub_migrate.go`) already re-keys a retired hub event-object layout in place, idempotently and resumably (a dual-read keeps unmigrated objects live, each object is verified by read-back on the new key before its legacy copy is deleted, and a fully migrated hub re-run reports nothing to do), with a fail-OPEN policy for anything it cannot safely parse or verify (kept, never deleted, on any ambiguity). A future breaking manifest bump should follow the same shape: an explicit, idempotent migration command, not a hard version-check wall.
...
Lifecycle:
- **Init** (`devstrap init`): the **founder** mints the workspace id; a second device does not mint its own but adopts the founder's with `devstrap init --join --workspace-id <id>` (`--workspace-id` implies `--join`; the R2/S3 hub keys under `workspaces/<workspace_id>/`, so a self-minted id keys a disjoint prefix — see the pairing runbook in `19_CLOUD_PROVISIONING_GUIDE.md` §E). Adopting is born-correct: a mismatch against an already-initialized store is refused with a remove-and-reinit remedy, never a post-hoc rewrite. Init does **not** mint a WCK (`P6-SEC-02`, shipped). Founding is deferred to the first `devstrap sync`: the founder/join gate in `runSyncCycle` mints epoch 1 (`EnsureBootstrap`) only when the hub is genuinely empty — **both** the pull and push cursors are 0 (this device has never observed hub content) **and** the pull returned zero raw objects (`EncryptedHub.PullStats.RawSeen == 0`; `RawSeen` alone only proves "nothing new after the pull cursor", so a keyless device whose pull cursor already advanced past quarantined hub events must not found) — and the device did not `init --join`. A device that JOINS an existing workspace (`init --join`, or any device whose first pull sees a non-empty hub) never self-mints a key nobody else holds, so it can never seal its pre-approval events under a never-granted WCK — the SEC-02 data loss. The joiner receives the fleet WCK via `devices approve` (`GrantAllEpochs`) on an existing device; approving another device from a never-synced **founder** still founds defensively, but a **joiner** approving a device while holding no key never self-mints (`grantWorkspaceKeyToApprovedDevice` is founder-gated — it warns and grants nothing, closing the last self-mint path a joiner could reach). WCKs are stored in the OS keychain / 0600 file fallback (`devicekeys.HybridStore`). The founder-side bootstrap has a guided one-command form, `devstrap up [root] --hub <url>` (`P7-PROD-01` slice 2), that folds `init` + `scan --adopt` + hub configuration + the founding `sync` into one idempotent, resumable sequence.
- **Approve** (`devices approve` / `enroll --approve`): gated on out-of-band **fingerprint confirmation** before any trust-state write (`P4-SEC-04`, shipped) — the full 256-bit fingerprint binding the device's signing key and age recipient is computed from the row/flags/code being approved and must be confirmed via `--fingerprint`, an interactive `yes`, or (non-TTY) a copy-paste remedy; a keyless placeholder row is refused outright (`SECU-05`). The exchanged values may ride in an unauthenticated one-paste pairing blob (`devices pairing-code`, `devstrap join`, `init --join --code`, `devices enroll --code`); integrity still comes from the derived fingerprint, never from the blob. **Pairing-code wire version** (`internal/pairing`, `P7-PROD-01`): the blob is a base64url JSON payload behind a `devstrap-pair<N>:` prefix that is authoritative for the version (the inner `v` field must agree, or the blob is rejected). **v1** (`devstrap-pair1:`, older binaries) carries workspace id, device id, name/os/arch, age recipient, and signing key — no fingerprint. **v2** (`devstrap-pair2:`, the current `Encode` output) adds two OPTIONAL fields: the fingerprint (derived at `Encode` from the same keys; `Decode` rejects a value that disagrees with the keys as corruption) and an optional hub URI (so `devstrap join` auto-configures a **remote** hub — `r2://`, `s3://`, `git+ssh://`, `git@host:path`). The embedded fingerprint is a convenience + corruption check, **not** authentication — the blob stays unauthenticated, so `join` auto-trusting it trusts the paste channel, and `join --fingerprint <fp>` is the opt-in high-assurance out-of-band compare. For the same reason, a carried **local** `file:`/`folder:` hub URI is never auto-applied by `join` — an unauthenticated blob must not be able to silently redirect a joiner's sync at an attacker-chosen local filesystem path; the operator confirms it by hand with `devstrap hub init <url>`. `Decode` still parses a v1 blob exactly; a blob whose prefix version exceeds this binary's still errors with the "created by a newer devstrap; upgrade" message. Once confirmed, `GrantAllEpochs(recipient)` wraps every held epoch's WCK to the newly-approved device and emits one `device.key.granted` event per epoch. The new device ingests them on its first pull and decrypts the entire history. On a keyless **joiner**, approve grants nothing (`grantWorkspaceKeyToApprovedDevice` is founder-gated) but the approved row still pins the device's keys and flips verification fail-closed — the `P4-SEC-04` founder-pinning ceremony a joiner runs before its first sync. The founder's half of the ceremony has a guided interactive wizard, `devstrap pair` (`P7-PROD-01` slice 2): at a TTY it prints this device's code + the exact `devstrap join` command, blocks on the joiner's pasted-back code, then runs the same `devices enroll --code --approve` confirmation + `sync` — a convenience over the manual steps, which stay the scripted/CI fallback (`pair` fast-fails without a terminal).
