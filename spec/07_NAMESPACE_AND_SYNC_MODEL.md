...
- **`enc.v2`** with a full-carrier AEAD AAD binding `ID || DeviceID || Seq || HLC || epoch` (`P6-SYNC-04`), so a carrier field cannot be swapped without failing decryption (the `kid` should join the AAD in the same break);
- ~~a keyring keyed by `(epoch, kid)`~~ — **shipped**: `kid = hex(sha256(wck))` (full digest) rides `DeviceKeyGrant` and the `enc.v1` envelope (as a JSON field outside the AAD for wire compat), and the keyring/keystore key by `(epoch, kid)` so self-minted colliding keys never alias (`P6-SEC-01`/`P6-SEC-02`);
