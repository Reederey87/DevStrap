...
- **Layer B — WIP recovery (Phase 1, push/fetch SHIPPED `P7-WIP-01`/`P7-WIP-02`):** `git stash create` (no worktree/index mutation, `internal/git.Runner.StashCreate`) → `git push origin <sha>:refs/devstrap/wip/<device_id>/<path_key>` over a raw refspec push (`internal/git.Runner.PushRef`, validated by a namespace-scoped `safeRefPath` distinct from the generic `safeBranchName`) → emit `repo.wip.pushed` (`internal/sync.NewWipPushedEvent`). Forge-agnostic. Apply is mirror-only into a sidecar `device_wip` table (migration `00030`; same no-FK rationale as `device_gitstate`/00029), and `repo.wip.pushed` sits at the same attributed-but-unverified trust tier as `repo.gitstate.observed` (neither is in `mustVerifyEvent` until Phase-2 device enrollment). `devstrap wip push <project>` is the write half (see `13_CLI_DAEMON_API.md` § *WIP commands* for the full CLI contract); `devstrap wip fetch <project> [--device <id>]` is machine B's read half — it always derives the ref to fetch canonically from device id + path_key (never trusting a stored/peer-supplied `Ref` string as the fetch target) and, without `--device`, discovers candidates from the synced `device_wip` mirror rather than probing the network. Not yet built: `wip status|show|apply|drop` — until those land, a fetched ref sits in local git storage with no way to inspect or materialize it into the worktree (`wip apply` will remain explicit-command-only, never a branch or base, per the invariant below).
