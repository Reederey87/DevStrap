...
### P6-SYNC-02 — Pull's skip classes leave no durable record; a skipped object retry-wedges its origin device

**Problem (re-based on the P5-SYNC-01 per-device cursor).** `EncryptedHub.Pull` still SKIPS `ParseEncryptedEnvelope` failures (incl. `ErrUnknownEnvelopeVersion`), retired enc.v1 traffic, anti-downgrade plaintext, and the three first-pass grant-skip branches, with only a log line. The old failure mode — the cursor silently advancing past the skipped event forever — is gone: the drop leaves a per-device seq gap, so that origin device's cursor holds and the object is retried every pull. The residuals: the retry wedge is unbounded and invisible (no durable record, no `status`/`doctor` surfacing, no classification between recoverable-after-upgrade (`ErrUnknownEnvelopeVersion`) and permanently-malformed junk), and the held stream's later events re-transfer every cycle.
