...
The model check **surfaced a second divergence** in the same `reconcileSamePath` lowest-coordinate root cause, this time **with no delete involved**: on a same-path/different-remote path where one remote carries multiple events at different HLCs, same-remote LWW keeps that remote's HIGHEST HLC while the cross-remote reconcile keeps the LOWEST coordinate, so the terminal winner flips by delivery order (e.g. `rB@4, rB@1, rA@1` folds to `rA@1` one way and `rB@4` the reverse). The event-set generator therefore excludes BOTH divergent classes (the delete residual above and this multi-event one), each pinned by a witness tripwire (`TestDecideDifferentRemote{Delete,MultiEvent}DivergesWitness`) that fails the day `reconcileSamePath` is made HLC-monotonic — the signal to remove the matching exclusion. The single fix for both is a `reconcileSamePath` HLC-monotonic winner (a tracked follow-up).
