...
- **Pull**: `EncryptedHub.Pull` primes the keyring, ingests in-batch grants in HLC order (so a new device obtains its WCKs before decrypting history), then decrypts `enc.v1` envelopes. Non-grant plaintext events from the hub are rejected as a downgrade (`ErrPlaintextEventFromHub`). A missing epoch key returns `ErrMissingWorkspaceKey` before advancing the pull cursor.
