...
**Status: wire format, hub snapshot plane, snapshot import + `ErrSnapshotRequired` recovery, and the `hub compact` producer are shipped; tombstone GC (`P4-SYNC-06`) and signed per-device sync acks (`P6-HUB-04` completion) land as the following PR of the same wave.**
