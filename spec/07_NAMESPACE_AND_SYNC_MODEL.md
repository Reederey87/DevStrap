...
Lifecycle:
- **Init** (`devstrap init`): does **not** mint a WCK (`P6-SEC-02`, shipped). Founding is deferred to the first `devstrap sync`: the founder/join gate in `runSyncCycle` mints epoch 1 (`EnsureBootstrap`) only when the hub is genuinely empty (`push cursor == 0` and the pull observed zero raw objects, `EncryptedHub.PullStats.RawSeen == 0`) and the device did not `init --join`. A device that JOINS an existing workspace (`init --join`, or any device whose first pull sees a non-empty hub) never self-mints a key nobody else holds, so it can never seal its pre-approval events under a never-granted WCK — the SEC-02 data loss. The joiner receives the fleet WCK via `devices approve` (`GrantAllEpochs`) on an existing device; approving another device from a never-synced **founder** still founds defensively, but a **joiner** approving a device while holding no key never self-mints (`grantWorkspaceKeyToApprovedDevice` is founder-gated — it warns and grants nothing, closing the last self-mint path a joiner could reach). WCKs are stored in the OS keychain / 0600 file fallback (`devicekeys.HybridStore`).
...
**Remaining (tracked as PR-3b follow-up).** The bare-integer-epoch overwrite (`IngestGrant`'s unconditional `StoreWCK` displacing a legacy self-mint) and the concurrent-`Rotate` collision are NOT yet closed. With founding deferred AND the approve path founder-gated, a joiner never self-mints on any command path; the residual is strictly a legacy already-self-minted device and same-epoch `Rotate` races. The fix is `kid`-addressed keys so colliding WCKs coexist instead of clobbering:
...
This `(epoch, kid)` keying is what makes `P6-SEC-01` steps (b) held-epoch overwrite refusal and (c) verified-epoch `CurrentKeyEpoch` gating safe (a legacy self-minted epoch-1 must stay displaceable by the founder grant until then), so the three land together in the follow-up.
