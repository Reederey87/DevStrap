...
1. **Contents:** code + tests + spec updates + a newest-first `spec/18_WORK_LOG.md` entry + `docs/audits/README.md` ledger reconciliation (shipped findings move to *Recently shipped*; the pass header's open count must equal its open-table row count). When a PR changes the **substance** of a `spec/*.md` file — a status claim, an inventory, an architecture or decision statement, anything a reader would treat as current — bump that file's `last_reviewed` frontmatter date to the PR date in the same PR. A cross-reference fix, typo, or link/rename touch is **not** substantive and must not bump the date (that would fake freshness). The drift gate proves only that a mapped spec was *touched* (`P5-DX-02`), never that its `last_reviewed` reflects the change, so keeping the date honest is the author's job; `spec/18_WORK_LOG.md` is append-only and exempt (its entries, not its `last_reviewed`, carry the history).
...
4. **Merge:** `gh pr merge --squash --auto` after green CI. Every review thread must be replied to **and resolved**, or auto-merge stays blocked. **CodeRabbit is currently inactive on this repo** — it last reviewed PR #225 (2026-07-24) and has posted on none of the fifteen PRs merged since, so treat its absence as expected rather than as a pending review. If it is reinstated, its threads fall under the same rule. The owner may bypass protection for hotfixes but should prefer the PR flow.
5. **Multi-PR waves:** merge serially and rebase each successor — `spec/18` and the ledger conflict by design. Keep all work-log entries, re-derive the ledger count from the table, and `grep -c '^<<<<<<<'` every resolved file before `git rebase --continue` — **anchored to line start**, because `spec/18` contains prose describing git's markers, so an unanchored grep always returns a nonzero count and is useless as a gate. Resolve EVERY block, not just the first: a single file can carry more than one, and `git add` will happily stage a file with markers still in it.
