...
func TestAgentSandboxSpecFailsClosedWithoutUserHome(t *testing.T) {
	t.Setenv("HOME", "")
	if _, err := agentSandboxSpec("/wt", "/tmp/run", "/log", nil, nil, agentSandboxLaunch{devstrapHome: "/dsh"}, "arun_test"); err == nil {
...
func TestAgentSandboxSpecAnchorsRealUserHome(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	spec, err := agentSandboxSpec("/wt", "/tmp/run", "/log", nil, nil, agentSandboxLaunch{devstrapHome: "/dsh", denyNetwork: true}, "arun_test")
	if err != nil {
