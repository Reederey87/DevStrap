...
func existingRealPaths(paths []string) []string {
	...
	for _, path := range paths {
		...
		// target, and ~/.ssh -> elsewhere must mask elsewhere. Missing deny
		// anchors are harmless in Seatbelt, but bwrap would fail with
		// "Can't mkdir" under the read-only root, so drop them here.
